...
echo deb http://archive.apache.org/dist/cassandra/debian 12x main | \
    sudo tee $CASSANDRA_SOURCES_LIST

wget -qO- https://archive.apache.org/dist/cassandra/KEYS | \
    sudo apt-key add - || true

# add the reddit ppa for some custom packages. Omega: TODO localize this external dependency
...
cat <<HERE > /etc/apt/preferences.d/artemis
Package: *
Pin: release o=LP-PPA-artemis
Pin-Priority: 600
HERE
