...
if "artemis" not in sys.list_keyspaces():
    print "creating keyspace 'artemis'"
    sys.create_keyspace("artemis", "SimpleStrategy", {"replication_factor": "1"})
    print "done"

if "permacache" not in sys.get_keyspace_column_families("artemis"):
    print "creating column family 'permacache'"
    sys.create_column_family("artemis", "permacache")
    print "done"
END
