...
from r2.models.gold import append_random_bottlecap_phrase, cbranches_lock
...
class AdminTools(object):
    ...
    def set_last_sr_ban(self, things):
        ...
        if by_srid:
            srs = Branch._byID(by_srid.keys(), data=True, return_dict=True)
    ...
    def engolden(self, account):
        ...
        trophy = Award.give_if_needed("artemis_gold", account,
    ...
    def degolden(self, account):
        Award.take_away("artemis_gold", account)
        account.gold = False
        account._commit()
...
def update_gold_users():
    ...
    renew_msg = _("[Click here for details on how to set up an "
                  ...
                  "rants, suggestions about artemis gold, please write "
    ...
    for account in all_gold_users():
        ...
        if days_left < 0:
            if account.pref_cartemis_autorenew:
                with cbranches_lock(account):
                    if account.gold_cbranches > 0:
                        admintools.adjust_gold_expiration(account, days=31)
                        account.gold_cbranches -= 1
                        account._commit()
                        continue

            admintools.degolden(account)

            subject = _("Your artemis gold subscription has expired.")
            message = _("Your subscription to artemis gold has expired.")
            message += "\n\n" + renew_msg
            ...
            if not already_warned:
                ...
                subject = _("Your artemis gold subscription is about to "
                            "expire!")
                message = _("Your subscription to artemis gold will be "
                            "expiring soon.")
                message += "\n\n" + renew_msg
                message = append_random_bottlecap_phrase(message)
...
def wiki_template(template_slug, sr=None):
    """Pull content from a branch's wiki page for internal use."""
    if not sr:
        try:
            sr = Branch._by_name(g.default_sr)
        except NotFound:
            return None
...
def send_system_message(user, subject, body, system_user=None,
    ...
    try:
        ...
        raise MessageError('artemis_inbox')
