...
MAX_RULES_PER_BRANCH = 10


class BranchRules(tdb_cassandra.View):
    _use_db = True
    ...
    @classmethod
    def create(self, branch, short_name, description, kind=None,
            created_utc=None):
        """Create a rule and append to the end of the priority list."""
        try:
            priority = len(list(self._cf.get(branch._id36)))
        except tdb_cassandra.NotFoundException:
            priority = 0

        if priority >= MAX_RULES_PER_BRANCH:
            return

        blob = self.get_rule_blob(short_name, description, priority,
            kind, created_utc)
        self._set_values(branch._id36, blob)

    @classmethod
    def remove_rule(self, branch, short_name):
        """Remove a rule and update priorities of remaining rules."""
        self._remove(branch._id36, [short_name])
        ...
        self._set_values(branch._id36, blobs)

    @classmethod
    def update(self, branch, old_short_name, short_name, description,
            kind=None):
        """Update the short_name or description of a rule."""
        rules = self._cf.get(branch._id36)
        if old_short_name != short_name:
            old_rule = rules.get(old_short_name, None)
            self._remove(branch._id36, [old_short_name])
        else:
    ...
    @classmethod
    def reorder(self, branch, short_name, priority):
        ...
        """
        rule_to_reorder = self.get_rule(branch, short_name)
    ...
    @classmethod
    def get_rule(self, branch, short_name):
