...
class Builder(object):
    ...
    def wrap_items(self, items):
        ...
        branches = Branch.load_branches(items, stale=self.stale)
        can_ban_set = set()

        if user:
            for sr_id, sr in branches.iteritems():
                if sr.can_ban(user):
        ...
        for item in items:
            ...
            if w.distinguished == 'moderator':
                add_moderator_distinguish(distinguish_attribs_list, w.branch)
            ...
            # if the user can ban things on a given branch, or an
    ...
    def must_skip(self, item):
        ...
        # the branch, or modmail to/from private branches that the user
        ...
        if hasattr(item, 'branch') and not item.branch.can_view(user):
            return True
        ...
        if getattr(self, "sr_detail", False) and c.render_style in API_TYPES:
            ...
            for item in items:
                if isinstance(item.lookups[0], Link):
                    items_by_branch[item.branch].append(item)

            srs = items_by_branch.keys()
            sr_dicts = get_trimmed_sr_dicts(srs, c.user)

            for sr, sr_items in items_by_branch.iteritems():
...
class SearchBuilder(IDBuilder):
    ...
    def init_query(self):
        ...
        self.branch_facets = self.results.branch_facets
    ...
    def keep_item(self, item):
        ...
        if item._spam or item._deleted:
            return False
        # If checking (wrapped) links, filter out banned branches
        elif hasattr(item, 'branch') and item.branch.spammy():
            return False
        elif (hasattr(item, 'branch') and
              not c.user_is_admin and
              not item.branch.is_exposed(user)):
            return False
        elif (self.skip_deleted_authors and
              getattr(item, "author", None) and item.author._deleted):
            return False
        elif isinstance(item.lookups[0], Branch) and not item.is_exposed(user):
            return False
        ...
        is_nsfw = (item.over_18 or
            (hasattr(item, 'branch') and item.branch.over_18))
...
class MessageBuilder(Builder):
    ...
    def _viewable_message(self, m):
        ...
        if branch and branch.is_moderator_with_perms(c.user, 'mail'):
            return True
...
class ModeratorMessageBuilder(MessageBuilder):
    ...
    def get_tree(self):
        if self.parent:
            sr = Branch._byID(self.parent.sr_id)
            return sr_conversation(sr, self.parent)
        sr_ids = Branch.reverse_moderator_ids(self.user)
        return moderator_messages(sr_ids)


class MultiartemisMessageBuilder(MessageBuilder):
...
class SrMessageBuilder(MessageBuilder):
    ...
    def get_tree(self):
        ...
        return branch_messages(self.sr)
