...
from vote import *
from report import *
from branch import DefaultSR, AllSR, Frontpage, Branch
...
class BannedListing(UserListing):
    ...
    def __init__(self, builder, show_jump_to=False, show_not_found=False,
        ...
        self.rules = rules.BranchRules.get_rules(c.site)
    ...
    def title(self):
        return _("users banned from"
                 " /r/%(branch)s") % dict(branch=c.site.name)
...
class WikiMayContributeListing(ContributorListing):
    ...
    def title(self):
        return _("approved wiki contributors"
                 " for /r/%(branch)s") % dict(branch=c.site.name)
...
class InvitedModListing(UserListing):
    ...
    remove_self_title = _('you are a moderator of this branch. %(action)s')
    ...
    def title(self):
        return _("invited moderators for"
                 " %(branch)s") % dict(branch=c.site.name)
...
class ModListing(InvitedModListing):
    ...
    @property
    def title(self):
        return _("moderators of /r/%(branch)s") % dict(branch=c.site.name)
...
class SearchListing(LinkListing):
    ...
    def listing(self, legacy_render_class=False, *args, **kwargs):
        ...
        if hasattr(self.builder, 'branch_facets'):
            self.branch_facets = self.builder.branch_facets
