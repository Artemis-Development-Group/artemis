...
restricted_namespaces = ('artemis/', 'config/', 'special/')
...
# Pages that get created automatically from the branch settings page
...
modactions = {
    ...
    "config/description": "Updated branch description",
    "config/sidebar": "Updated branch sidebar",
...
class WikiPage(tdb_cassandra.Thing):
    """ Contains permissions, current content (markdown), branch, and current revision (ID)
        Key is branch-pagename """
...
class WikiPagesBySR(tdb_cassandra.DenormalizedView):
    """ Associate revisions with branches, store only recent """
    _use_db = True
    _connection_pool = 'main'
    _view_of = WikiPage
