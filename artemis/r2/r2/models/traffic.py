...
###############################################################################
"""
These models represent the traffic statistics stored for branches and
...
def campaign_history(cls, codenames, start, stop):
    ...
    q = (Session.query(cls)
                ...
                .filter(cls.date.in_(time_points))
                .order_by(cls.date))
    return [(r.date, r.codename, r.branch, (r.unique_count,
                                               r.pageview_count))
            for r in q.all()]
...
class PageviewsByBranch(Base):
    """Pageviews within a branch (i.e. /r/something/...)."""

    __tablename__ = "traffic_branches"

    branch = Column(String(), nullable=False, primary_key=True)
    date = Column(DateTime(), nullable=False, primary_key=True)
    interval = Column(String(), nullable=False, primary_key=True)
    unique_count = Column("unique", Integer())
    ...
    @classmethod
    @memoize_traffic(time=3600)
    def history(cls, interval, branch):
        time_points, q = make_history_query(cls, interval)
        q = q.filter(cls.branch == branch)
        return fill_gaps(time_points, q, "unique_count", "pageview_count")

    @classmethod
    @memoize_traffic(time=3600 * 6)
    def top_last_month(cls, num=None):
        return top_last_month(cls, "branch", num=num)

    @classmethod
    @memoize_traffic(time=3600 * 6)
    def last_month(cls, srs):
        ids = [sr.name for sr in srs]
        return top_last_month(cls, "branch", ids=ids)


class PageviewsByBranchAndPath(Base):
    """Pageviews within a branch with action included.

    `srpath` is the branch name, a dash, then the controller method called
    to render the page the user viewed. e.g. Artemis-GET_listing. This is
    useful to determine how many pageviews in a branch are on listing pages,
...
class SubscriptionsByBranch(Base):
    """Subscription statistics for branches.
