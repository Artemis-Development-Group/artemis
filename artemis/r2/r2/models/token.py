...
class OAuth2Scope:
    scope_info = {
        ...
        "cbranches": {
            "id": "cbranches",
            "name": _("Spend artemis gold cbranches"),
            "description": _("Spend my artemis gold cbranches on giving "
                "gold to other users."),
        },
        "edit": {
            "id": "edit",
        ...
        "flair": {
            ...
            "description": _("Select my branch flair. "
        ...
        "identity": {
            "id": "identity",
            "name": _("My Identity"),
            "description": _("Access my artemis username and signup date."),
        },
        "modcontributors": {
            "id": "modcontributors",
            ...
            "description": _(
                ...
                "ban/unban or mute/unmute users from branches I moderate."
            ),
        },
        "modflair": {
            "id": "modflair",
            ...
            "description": _(
                "Manage and assign flair in branches I moderate."),
        },
        "modposts": {
            "id": "modposts",
            ...
            "description": _(
                ...
                " in branches I moderate."),
        },
        "modconfig": {
            "id": "modconfig",
            "name": _("Moderate Branch Configuration"),
            "description": _(
                "Manage the configuration, sidebar, and CSS"
                " of branches I moderate."),
        },
        "modlog": {
            "id": "modlog",
            "name": _("Moderation Log"),
            "description": _(
                "Access the moderation log in branches I moderate."),
        },
        "modothers": {
            "id": "modothers",
            "name": _("Invite or remove other moderators"),
            "description": _(
                "Invite or remove other moderators from branches I moderate."
            ),
        },
        "modself": {
            "id": "modself",
            "name": _("Make changes to your branch moderator "
                      "and contributor status"),
            "description": _(
                "Accept invitations to moderate a branch. Remove myself as "
                "a moderator or contributor of branches I moderate or "
                "contribute to."
            ),
        },
        "modtraffic": {
            "id": "modtraffic",
            "name": _("Branch Traffic"),
            "description": _("Access traffic stats in branches I moderate."),
        },
        "modwiki": {
            "id": "modwiki",
        ...
        "mybranches": {
            ...
            "name": _("My Branches"),
            "description": _(
                "Access the list of branches I moderate, contribute to,"
                " and subscribe to."),
        },
        "privatemessages": {
            "id": "privatemessages",
        ...
        "subscribe": {
            ...
            "description": _('Manage my branch subscriptions. Manage '
    ...
    def __init__(self, scope_str=None, branches=None, scopes=None):
        if scope_str:
            self._parse_scope_str(scope_str)
        elif branches is not None or scopes is not None:
            self.branch_only = bool(branches)
            self.branches = branches
            self.scopes = scopes
        else:
            self.branch_only = False
            self.branches = set()
            self.scopes = set()
    ...
    def _parse_scope_str(self, scope_str):
        ...
        if sep:
            self.branch_only = True
            self.branches = set(srs.split('+'))
        else:
            self.branch_only = False
            self.branches = set()
    ...
    def __str__(self):
        if self.branch_only:
            sr_part = '+'.join(sorted(self.branches)) + ':'
        else:
            sr_part = ''
        return sr_part + ' '.join(sorted(self.scopes))

    def has_access(self, branch, required_scopes):
        if self.FULL_ACCESS in self.scopes:
            return True
        if self.branch_only and branch not in self.branches:
            return False
    ...
    def merge_scopes(cls, scopes):
        """Return a by-branch dict representing merged OAuth2Scopes.

        Takes an iterable of OAuth2Scopes. For each of those,
        if it defines scopes on multiple branches, it is split
        into one OAuth2Scope per branch. If multiple passed in
        OAuth2Scopes reference the same scopes, they'll be combined.

        """
        merged = {}
        for scope in scopes:
            srs = scope.branches if scope.branch_only else (None,)
            for sr in srs:
                if sr in merged:
                    merged[sr].scopes.update(scope.scopes)
                else:
                    new_scope = cls()
                    new_scope.branches = {sr}
                    new_scope.scopes = scope.scopes
                    if sr is not None:
                        new_scope.branch_only = True
                    merged[sr] = new_scope
        return merged
