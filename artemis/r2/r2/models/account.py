...
class Account(Thing):
    ...
    def has_interacted_with(self, sr):
        try:
            r = BranchParticipationByAccount.fast_query(self, [sr])
    ...
    def incr_karma(self, kind, sr, amt):
        ...
        # having a massive number of branch-karma attributes
        if self.disable_karma:
            return

        if sr.name.startswith('_'):
            g.log.info("Ignoring karma increase for branch %r" % (sr.name,))
            return
    ...
    def all_karmas(self, include_old=True):
        """Get all of the user's branch-specific karma totals.

        Returns an OrderedDict keyed on branch name and containing
    ...
    def can_create_branch(self):
        hook = hooks.get_hook("account.can_create_branch")
    ...
    def is_mutable(self, branch):
        # Don't allow muting of other mods in the branch
        if branch.is_moderator(self):
            return False

        # Don't allow muting of u/artemis or u/AutoModerator
    ...
    def branches(self):
        ...
        return Branch.user_branches(self)
    ...
    def use_branch_style(self, sr):
        """Return whether to show branch stylesheet depending on
        individual selection if available, else use pref_show_stylesheets"""
        # if FakeBranch, there is no stylesheet
    ...
    def set_branch_style(self, sr, use_style):
        if hasattr(sr, '_id'):
    ...
    def can_flair_in_sr(self, user, sr):
        """Return whether a user can set this one's flair in a branch."""
    ...
    def set_flair(self, branch, text=None, css_class=None, set_by=None,
            log_details="edit"):
        log_details = "flair_%s" % log_details
        if not text and not css_class:
            # set to None instead of potentially empty strings
            text = css_class = None
            branch.remove_flair(self)
            log_details = "flair_delete"
        elif not branch.is_flair(self):
            branch.add_flair(self)

        setattr(self, 'flair_%s_text' % branch._id, text)
        setattr(self, 'flair_%s_css_class' % branch._id, css_class)
        self._commit()

        if set_by and set_by != self:
            ModAction.create(branch, set_by, action='editflair',
    ...
    def gold_will_autorenew(self):
        return (self.has_gold_subscription or
                (self.pref_cartemis_autorenew and self.gold_cbranches > 0))
    ...
    def get_style_override(self):
        """Return the branch selected for artemis theme.

        If the user has a theme selected and enabled and also has
        the feature flag enabled, return the branch name.
        ...
        # Artemis themes is not enabled for this user
        if not feature.is_enabled('stylesheets_everywhere'):
            return None
...
class BlockedBranchesByAccount(tdb_cassandra.DenormalizedRelation):
    _use_db = True
    _last_modified_name = 'block_branch'
...
def deleted_account_cleanup(data):
    ...
    for account_id36 in data.itervalues():
        ...
        for rel_type, description in rel_removal_descriptions.iteritems():
            try:
                ids_fn = getattr(Branch, "reverse_%s_ids" % rel_type)
                sr_ids = ids_fn(account)

                sr_names = []
                srs = Branch._byID(sr_ids, data=True, return_dict=False)
                for branch in srs:
                    remove_fn = getattr(branch, "remove_" + rel_type)
                    remove_fn(account)
                    sr_names.append(branch.name)
...
class BranchParticipationByAccount(tdb_cassandra.DenormalizedRelation):
    _use_db = True
    ...
    @classmethod
    def mark_participated(cls, account, branch):
        cls.create(account, [branch])


class QuarantinedBranchOptInsByAccount(tdb_cassandra.DenormalizedRelation):
    _use_db = True
    _last_modified_name = 'QuarantineBranchOptin'
    _read_consistency_level = tdb_cassandra.CL.QUORUM
    _write_consistency_level = tdb_cassandra.CL.QUORUM
    _extra_schema_creation_args = {
        "key_validation_class": ASCII_TYPE,
    ...
    @classmethod
    def opt_in(cls, account, branch):
        if branch.quarantine:
            cls.create(account, branch)

    @classmethod
    def opt_out(cls, account, branch):
        if branch.is_subscriber(account):
            branch.remove_subscriber(account)
        cls.destroy(account, branch)

    @classmethod
    def is_opted_in(cls, user, branch):
        try:
            r = cls.fast_query(user, [branch])
        except tdb_cassandra.NotFound:
            return False
        return (user, branch) in r
