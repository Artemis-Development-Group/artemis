...
!function(r, undefined) {
  ...
  /** How long to wait for the branch rules to load. @const */
  var RULES_TIMEOUT_MS = 10000;


  /** This looks like a branch name. @const */
  var BRANCH = /^(?:#|\/?r\/)(.*)/;
  ...
  var mc = r.messagecompose = {
    ...
    init: function() {
      ...
      if (mc.dom.$form.length) {
        ...
        // submission (post_form in artemis.js) so that it does't interfere with the
        ...
        if (mc.dom.$to.length) {
          ...
          // branch, so we have to load them up front.
          mc.onToUpdate();
        }
    ...
    /**
     * Loads the rules for the given branch.
     * Returns a promise that resolves with the rules JSON object or rejects if the
     * branch doesn't exist or otherwise fails to load.
     *
     * @private
     * @param {string} sr The name of the branch.
     * @return {Promise<{
     *     sr_name: string,
     ...
     *  }>}
     */
    loadBranchRules: function(sr) {
      var url = '/r/' + sr + '/about/rules.json';

      // If we're already loading this branch, continue, otherwise, abort
    ...
    /**
     * Renders the rules of the branch into the subject field, while maintaining
     * any user entered value.
     *
     * @private
     * @param {!Object} rulesJson
     */
    renderBranchSubject: function(rulesJson) {
    ...
    /**
     * Changes the state of the form to be correct for a non-branch recipient.
     * @private
     */
    ...
    onToUpdate: function() {
      ...
      // Is this probably a branch?
      var m = BRANCH.exec(toValue);
      if (m) {
        mc.loadBranchRules(m[1])
          .then(mc.renderBranchSubject, mc.renderGeneralSubject);
      } else {
  ...
  r.hooks.get('artemis-init').register(r.messagecompose.init);
