...
requires Backbone
requires r.errors
requires r.models.BranchRule
requires r.models.BranchRuleCollection
requires r.ui.TextCounter
 */

!function(r, Backbone, undefined) {
  var BranchRuleBaseView = Backbone.View.extend({
    ...
    events: {
      'click .branch-rule-delete-button': function onDelete(e) {
        e.preventDefault();
        this.delete();
      },

      'click .branch-rule-edit-button': function onEdit(e) {
        e.preventDefault();
        this.edit();
      },

      'click .branch-rule-cancel-button': function onCancel(e) {
        e.preventDefault();
        this.cancel();
      },
    ...
    delegateEvents: function() {
      BranchRuleBaseView.__super__.delegateEvents.apply(this, arguments);
  ...
  var BranchRuleView = BranchRuleBaseView.extend({
    DELETING_STATE: 'deleting',

    initialize: function(options) {
      BranchRuleView.__super__.initialize.apply(this, arguments);
      this.ruleTemplate = options.ruleTemplate;
    },

    delegateEvents: function() {
      BranchRuleView.__super__.delegateEvents.apply(this, arguments);
    ...
    render: function() {
      BranchRuleView.__super__.render.call(this);

      if (this.state === this.DELETING_STATE) {
        this.$el.addClass('mod-action-deleting');
        this.$el.find('.branch-rule-delete-confirmation').removeAttr('hidden');
        this.$el.find('.branch-rule-buttons button').attr('disabled', true);
      }
      if (this.state === this.EDITING_STATE) {
  ...
  var AddBranchRuleView = BranchRuleBaseView.extend({
    DISABLED_STATE: 'disabled',

    initialize: function(options) {
      AddBranchRuleView.__super__.initialize.apply(this, arguments);
      this.collection = options.collection;
      this.initializeNewModel();
      this.$collapsedDisplay = this.$el.find('.branch-rule-add-form-buttons');
      this.$maxRulesNotice = this.$collapsedDisplay.find('.branch-rule-too-many-notice');
    ...
    delegateEvents: function() {
      AddBranchRuleView.__super__.delegateEvents.apply(this, arguments);
    ...
    render: function() {
      ...
      AddBranchRuleView.__super__.render.call(this);
  ...
  var BranchRulesPage = Backbone.View.extend({
    initialize: function(options) {
      this.ruleTemplate = options.ruleTemplate;
      this.formTemplate = options.formTemplate;
      var collectionOptions = {
        branchName: r.config.post_site,
        branchFullname: r.config.cur_site,
      };
      this.collection = new r.models.BranchRuleCollection(null, collectionOptions);

      this.newRuleForm = new AddBranchRuleView({
        el: options.addForm,
      ...
      var ruleItems = this.$el.find('.branch-rule-item').toArray();
      ruleItems.forEach(function(el) {
        var model = this.createBranchRuleModel(el);
        this.createBranchRuleView(el, model);
      }, this);
    ...
    delegateEvents: function() {
      BranchRulesPage.__super__.delegateEvents.apply(this, arguments);

      this.listenTo(this.newRuleForm, 'success', function(model) {
        var props = model.toJSON();
        var newModel = new r.models.BranchRule(props);
    ...
    createBranchRuleModel: function(el) {
      var $el = $(el);

      return new r.models.BranchRule({
        priority: parseInt($el.data('priority'), 10),
        short_name: $el.find('.branch-rule-title').text(),
        description: $el.data('description'),
        description_html: $el.find('.branch-rule-description').html(),
        kind: $el.data('kind'),
      });
    },

    createBranchRuleView: function(el, model) {
    ...
    addNewRule: function(model) {
      var el = $.parseHTML('<div class="branch-rule-item"></div>')[0];
      var view = this.createBranchRuleView(el, model);
      view.render();
      this.$el.append(el);
    },
    ...
    _updateRuleCache: function() {
      try {
        ...
        rulesCache[this.collection.branchFullname] = newRules;
  ...
  $(function() {
    var $page = $('.branch-rules-page');

    if (!$page.hasClass('editable')) {
      return;
    }

    var ruleTemplate = document.getElementById('branch-rule-template');
    var formTemplate = document.getElementById('branch-rule-form-template');
    var addForm = document.getElementById('branch-rule-add-form');
    var ruleList = document.getElementById('branch-rule-list');

    if (!ruleTemplate || !formTemplate) {
      throw 'Branch rule templates not found!';
    }
