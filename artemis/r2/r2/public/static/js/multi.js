r.multi = {
    init: function() {
        ...
        if (detailsEl.length) {
            ...
            var branchList = new r.multi.BranchList({
                model: multi,
                el: detailsEl
            })
...
r.multi.MultiArtemisList = Backbone.Collection.extend({
...
r.multi.MultiArtemis = Backbone.Model.extend({
    idAttribute: 'path',
    url: function() {
    ...
    initialize: function(attributes, options) {
        ...
        this.branches = new r.multi.MultiArtemisList(this.get('branches'), {
            url: this.url() + '/r/',
            parse: true
        })
        this.on('change:branches', function(model, value) {
            this.branches.set(value, {parse: true})
        }, this)
        this.branches.on('request', function(model, xhr, options) {
    ...
    addBranch: function(names, options) {
        names = r.utils.tup(names)
        if (names.length == 1) {
            this.branches.create({name: names[0]}, options)
        } else {
            ...
            // temporarily swap out the branches collection so we can
    ...
    removeBranch: function(name, options) {
        this.branches.getByName(name).destroy(options)
    },
    ...
    getBranchNames: function() {
        return this.branches.pluck('name')
    }
})

r.multi.MyMultiCollection = Backbone.Collection.extend({
    url: '/api/multi/mine',
    model: r.multi.MultiArtemis,
    comparator: function(model) {
...
r.multi.MultiBranchItem = Backbone.View.extend({
    ...
    removeBranch: function(ev) {
        this.options.multi.removeBranch(this.model.get('name'))
    }
})

r.multi.BranchList = Backbone.View.extend({
    events: {
        'submit .add-sr': 'addBranch'
    },

    initialize: function() {
        this.listenTo(this.model.branches, 'add', this.addOne)
        this.listenTo(this.model.branches, 'remove', this.removeOne)
        this.listenTo(this.model.branches, 'sort', this.resort)
        new r.ui.ConfirmButton({el: this.$('button.delete')})

        this.listenTo(this.model.branches, 'add remove', function() {
        ...
        this.itemView = this.options.itemView || r.multi.MultiBranchItem
        this.itemViews = {}
        this.bubbleGroup = {}
        this.$('.branches').empty()
        this.model.branches.each(this.addOne, this)
    },
    ...
    addOne: function(sr) {
        ...
        this.$('.branches').append(view.render().$el)
    },

    resort: function() {
        this.model.branches.each(function(sr) {
            this.itemViews[sr.id].$el.appendTo(this.$('.branches'))
        }, this)
    },
...
r.multi.MultiDetails = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        this.listenTo(this.model.branches, 'add remove reset', this.render)
    ...
    initRecommendations: function(recsEl) {
        ...
        // fetch initial data
        if (!this.model.branches.isEmpty()) {
            recs.fetchForSrs(this.model.getBranchNames())
        }
 
        // update recs when multi changes
        this.listenTo(this.model.branches, 'add remove reset',
            function() {
                var srNames = this.model.getBranchNames()
                recs.fetchForSrs(srNames)
            })
        // update multi when a rec is selected
        this.recsView.bind('recs:select',
            function(data) {
                this.model.addBranch(data['srName'])
            }, this)
    },
    ...
    render: function() {
        ...
        this.$('.count').text(this.model.branches.length)
...
r.multi.MultiAddNoticeBubble = r.ui.Bubble.extend({
    ...
    render: function() {
        this.$el.html(this.template({
            ...
            add_multi_sr: r._('add a branch to your multi.')
        }))
    }
})
...
r.multi.MultiSubscribeBubble = r.ui.Bubble.extend({
    ...
    render: function() {
        ...
        r.multi.mine.chain()
            .sortBy(function(multi) {
                // sort multibranches containing this branch to the top.
                return multi.branches.getByName(this.options.srName)
            }, this)
            .each(function(multi) {
                content.append(this.itemTemplate({
                    name: multi.get('name'),
                    path: multi.get('path'),
                    checked: multi.branches.getByName(this.options.srName)
                             ? 'checked' : '',
    ...
    toggleSubscribed: function(ev) {
        ...
        if (checkbox.is(':checked')) {
            multi.addBranch(this.options.srName)
        } else {
            multi.removeBranch(this.options.srName)
        }
    }
})
...
r.multi.MultiCreateForm = Backbone.View.extend({
    ...
    _createMulti: function(name) {
        var newMulti = new r.multi.MultiArtemis({
