!function(r) {
  ...
  var postSharingTemplate = _.template(
    ...
    '<div class="post-sharing-email-form post-sharing-form" ref="$emailForm">' +
      '<p class="post-sharing-label">' +
        ...
        '<span ref="$emailFormPMLabel">' + _.escape(r._('Share via private message on artemis as %(username)s').format({ username: r.config.logged })) + '</span>' +
      '</p>' +
      '<div class="c-form-group">' +
        '<input class="post-sharing-recipient-input c-form-control" ' +
               ...
               'data-placeholder-artemis-pm="username">' +
  ...
  r.ui.PostSharing = Backbone.View.extend({
    ...
    shareToFacebook: function() {
      var artemisUrl = this.getShareLink('facebook');
    ...
    shareToTwitter: function() {
      var artemisUrl = this.getShareLink('twitter');
      var twitterHandle = 'artemis'
    ...
    shareToTumblr: function() {
      var artemisUrl = this.getShareLink('tumblr');
      var title = this.thingData.title;
      var shareParams = {
        canonicalUrl: artemisUrl,
        posttype: 'link',
        title: title,
      };
    ...
    render: function() {
      ...
      if ('selectedOption' in changed) {
          ...
          this.refs.$emailFormPMLabel.toggle(selectedOption === 'artemis-pm');

          this.refs.$shareTo.attr('placeholder', this.refs.$shareTo.data('placeholder-' + selectedOption));

        if (selectedOption === 'email' || selectedOption === 'artemis-pm') {
  ...
  $(function() {
    $('body').on('click', '.post-sharing-button', function(e) {
      ...
      if (r.config.logged && !r.config.user_in_timeout) {
        shareOptions.push({
          name: 'artemis-pm',
          tooltip: r._('Private Message a Friend on Artemis'),
        });
      }
