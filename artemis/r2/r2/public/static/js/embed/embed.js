;(function(App, r, window, undefined) {
  ...
  var RE_HOST = /^https?:\/\/([^\/|?]+).*/;
  var config = window.ARTEMIS_EMBED_CONFIG;
  ...
  function clipComments() {
    ...
    function expandComment(e) {
      ...
      el.parentNode.className =
        el.parentNode.className.replace(' artemis-embed-comment-fade', '');
    }
    ...
    for (var i = 0, l = blockquotes.length; i < l; i++) {
      if (blockquotes[i].clientHeight > height + flex) {
        ...
        blockquotes[i].parentNode.className += ' artemis-embed-comment-fade';
