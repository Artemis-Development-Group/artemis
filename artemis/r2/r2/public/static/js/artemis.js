...
function subscribe(artemis_name) {
    return function() { 
        if (r.config.logged) {
            if (r.config.cur_site == artemis_name) {
                $('body').addClass('subscriber');
            }
            $.things(artemis_name).find(".entry").addClass("likes");
            $.request("subscribe", {sr: artemis_name, action: "sub"});
            r.analytics.fireUITrackingPixel("sub", artemis_name, {"has_subd": r.config.has_subscribed})
        }
    };
};

function unsubscribe(artemis_name) {
    return function() { 
        if (r.config.logged) {
            if (r.config.cur_site == artemis_name) {
                $('body').removeClass('subscriber');
            }
            $.things(artemis_name).find(".entry").removeClass("likes");
            $.request("subscribe", {sr: artemis_name, action: "unsub"});
            r.analytics.fireUITrackingPixel("unsub", artemis_name)
        }
    };
};

function quarantine_optout(branch_name) {
    return function() {
        if (r.config.logged) {
            $.request("quarantine_optout", {sr: branch_name});
            $.redirect("/");
        }
    };
};
...
function highlight_artemis(item) {
...
$(function() {
        ...
        $('.search-result-listing')
          .find('.search-title, .search-link, .search-branch-link, .search-result-body')
          .highlight(query);
        ...
        $(".artemis-themes .theme").click(function() {
