$(function() {
  var sessionStorageKey = 'branch-rules';
  ...
  function init() {
    var branchRulesTemplate = _getTemplate('branch-rules-report-template');
    var branchDefaultTemplate = _getTemplate('branch-default-report-template');
    var artemisTemplate = _getTemplate('artemis-report-template');
    var reasonTemplate = _getTemplate('report-reason-template');

    templates = {
      branchRules: function(data) {
        var rulesStr = data.rules.map(reasonTemplate).join('\n');
        var formStr = branchRulesTemplate(data);
      ...
      branchDefault: function(data) {
        var formStr = branchDefaultTemplate(data);
        var formEl = $.parseHTML(formStr);
        return formEl;
      },

      artemis: function(data) {
        var formStr = artemisTemplate(data);
        var formEl = $.parseHTML(formStr);
        return formEl;
      },
    };
  ...
  function renderFromTemplate(data, thingType) {
    var hasBranch = !!data.sr_name;
    var hasRules = data.rules && data.rules.length > 0;
    var template;
    var templateData;

    if (!hasBranch) {
      template = templates.artemis;
    } else if (hasRules) {
      template = templates.branchRules;
    } else {
      template = templates.branchDefault;
    }
  ...
  function openReportForm(e) {
    ...
    var srFullname = $thing.data('branch-fullname');
    ...
    if (useHtmlAPI) {
      ...
      // if no branch, render the artemis form (never needs to hit API)
      var formData = { fullname: attrs.thing };
      var form = renderFromTemplate(formData, thingType);
      showForm($reportForm, form);
    } else if (srFullname in cachedRules) {
      // render from cached if available (only needs to hit API once per branch)
  ...
  r.hooks.get('setup').register(function() {
    ...
    // temp fix for broken branch-report-form
    $('div.content').on('submit', '.branch-report-form', function(e) {
