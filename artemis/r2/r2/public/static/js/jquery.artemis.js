/* The artemis extension for jquery.  This file is intended to store
...
$.unsafe = function(text) {
    /* inverts websafe filtering of artemis app. */
    if(typeof(text) == "string") {
        text = text.replace(/&quot;/g, '"')
            .replace(/&gt;/g, ">").replace(/&lt;/g, "<")
            .replace(/&amp;/g, "&");
...
/* customized requests (formerly artemisRequest) */
...
$.request = function(op, parameters, worker_in, block, type, 
    ...
    /* 
       Uniquitous artemis AJAX poster.  Automatically addes
       handleResponse(action) worker to deal with the API result.  The
       current branch (r.config.post_site) and the user's modhash
    ...
    /* set the branch name if there is one */
...
$.fn.thing_id = function() {
    /* Returns the (artemis) ID of the current element's thing */
...
$.apply_stylesheet_url = function(cssUrl, srStyleEnabled) {
  var sheetTitle = 'applied_branch_stylesheet';
...
/* old artemis-specific cookie functions */
