...
var exports = r.sponsored = {
    ...
    setup_collection_selector: function() {
        ...
        var $branchList = $('.collection-branch-list ul');
        var $collectionLabel = $('.collection-branch-list .collection-label');
        var $frontpageLabel = $('.collection-branch-list .frontpage-label');

        var branchNameTemplate = _.template('<% _.each(sr_names, function(name) { %>'
            + ' <li><%= name %></li> <% }); %>');
        var render_branch_list = _.bind(function(collection) {
            if (collection === 'none' ||
                    typeof this.collectionsByName[collection] === 'undefined') {
                return '';
            }
            else {
                return branchNameTemplate(this.collectionsByName[collection]);
            }
        }, this);
        ...
        this.collapse_collection_selector = function collapse_widget() {
            ...
            var branchListItems = render_branch_list(val);
            $branchList.html(branchListItems);
            if (val === 'none') {
    ...
    setup_collections: function(collections, defaultValue) {
        ...
        this.collections = [{
            name: 'none',
            sr_names: null,
            description: 'influencers on artemis’s highest trafficking page',
        }].concat(collections || []);
        ...
        var template = _.template('<label class="form-group">'
          ...
          + '    <span class="label"><% print(name === \'none\' ? \'Artemis front page\' : name) %></span>'           + '    <small class="description"><%= description %></small>'
          + '  </div>'
          + '</label>');
    ...
    get_cpm: function($form) {
        ...
        var isBranch = $form.find('input[name="targeting"][value="one"]').is(':checked');
        var collectionVal = $form.find('input[name="collection"]:checked').val();
        var isFrontpage = !isBranch && collectionVal === 'none';
        var isCollection = !isBranch && !isFrontpage;
        var sr = isBranch ? $form.find('*[name="sr"]').val() : '';
        ...
        if (isFrontpage) {
            ...
            prices.push(this.priceDict.BRANCH[sr] || this.priceDict.BRANCH_DEFAULT);
        }
    ...
    get_targeting: function($form) {
        var isBranch = $form.find('input[name="targeting"][value="one"]').is(':checked'),
            ...
            isFrontpage = !isBranch && collectionVal === 'none',
            isCollection = !isBranch && !isFrontpage,
            type = isFrontpage ? 'frontpage' : isCollection ? 'collection' : 'branch',
            sr = isBranch ? $form.find('*[name="sr"]').val() : '',
            ...
            isValid = isFrontpage || (isBranch && sr) || (isCollection && collection);
    ...
    get_campaigns: function($list, $form) {
        ...
        function mapBranch(name, branches) {
            branches[name] = 1;
        }

        function getBranchesByCollection(name) {
            return collections[name] && collections[name].sr_names || null;
        }

        function mapCollection(name, branches) {
            var branchNames = getBranchesByCollection(name);
            if (branchNames) {
                _.each(branchNames, function(branchName) {
                    mapBranch(branchName, branches);
                });
            }
        }
        ...
        _.each(campaignRows, function(row) {
            ...
            var mappingFunction = isCollection ? mapCollection : mapBranch;
            var budget = parseFloat(data.total_budget_dollars, 10);

            if (data.is_auction === 'True') {
                auctionCampaigns++;
                mappingFunction(data.targeting, auctionBranches);
                totalAuctionBudgetDollars += budget;
            } else {
                fixedCPMCampaigns++;
                mappingFunction(data.targeting, fixedCPMBranches);
        ...
        return {
            ...
            fixedCPMBranches: fixedCPMBranches,
            auctionBranches: _.keys(auctionBranches),
            fixedCPMBranches: _.keys(fixedCPMBranches),
    ...
    auction_dashboard_help_template: _.template('<p>there '
        ...
        + '<%= auctionBranches.length %> branch'
        + '<% auctionBranches.length > 1 && print("s") %></p>'),
    ...
    fixed_cpm_dashboard_help_template: _.template('<p>there '
        ...
        + '<%= fixedCPMBranches.length %> branch'
        + '<% fixedCPMBranches.length > 1 && print("s") %>, amounting to a '
    ...
    branch_targeting: function() {
        $('.branch-targeting').find('*[name="sr"]').prop("disabled", false).end().slideDown();
        $('.collection-targeting').find('*[name="collection"]').prop("disabled", true).end().slideUp();
        this.render()
    },

    collection_targeting: function() {
        $('.branch-targeting').find('*[name="sr"]').prop("disabled", true).end().slideUp();
...
function create_campaign() {
    ...
    cancel_edit(function() {
            ...
            $("#campaign")
                ...
                .find(".branch-targeting").hide().end()
