r.analytics = {
  init: function() {
    // these guys are relying on the custom 'onshow' from jquery.artemis.js
    $(document).delegate(
    ...
    if (r.config.user_id) {
      ...
      var tracker = new artemislib.Tracker();
  ...
  expandoEvent: function(actionName, targetData) {
    ...
    if ('branchName' in targetData) {
      payload['sr_name'] = targetData.branchName;
    }

    if ('branchFullname' in targetData) {
      payload['sr_id'] = r.utils.fullnameToId(targetData.branchFullname);
    }
...
r.analytics.breadcrumbs = {
  selector: '.thing, .side, .sr-list, .srdrop, .tagline, .md, .organic-listing, .gadget, .sr-interest-bar, .trending-branches, a, button, input',
  maxLength: 3,
  sendLength: 2,
