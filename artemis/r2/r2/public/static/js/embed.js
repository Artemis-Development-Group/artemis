;(function($, undefined) {
    ...
    var embedBodyTemplate = _.template(
      ...
      '<div class="c-checkbox">' +
          ...
          '<div id="live-help" class="c-help-block c-toggle-content">' +
            '<p>' +
              _.escape(r._('When checked, if an embedded comment is later edited, the embedded comment text will be replaced by a link back to the current version of the comment on artemis.')) +
            '</p>' +
            '<p>' +
    ...
    $('body').on('click', '.embed-comment', function(e) {
      ...
      popup.on('show.r.popup', function() {
        $preview.find('.artemis-embed').hide();
      });
