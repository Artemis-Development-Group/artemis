...
try:
    ...
    from r2.controllers.artemis_base import ArtemisController, UnloggedUser
    ...
    from r2.models.branch import DefaultSR, Branch
except Exception, e:
    if g.debug:
...
'''<html>
  <head>
    <title>artemis broke!</title>
  </head>
  <body>
    <div style="margin: auto; text-align: center">
      <p>
        <a href="/">
          <img border="0" src="%s" alt="you broke artemis" />
        </a>
      </p>
      <p>
        %s
...
class ErrorController(ArtemisController):
    ...
    # List of admins to blame (skip the first admin, "artemis")
    # If list is empty, just blame "an admin"
    admins = g.admins[1:] or ["an admin"]
    def __before__(self):
        try:
            c.error_page = True
            ArtemisController.__before__(self)
    ...
    def __after__(self): 
        try:
            ArtemisController.__after__(self)
        except Exception as e:
            handle_awful_failure("ErrorController.__after__: %r" % e)

    def __call__(self, environ, start_response):
        try:
            return ArtemisController.__call__(self, environ, start_response)
        except Exception as e:
    ...
    def send404(self):
        if 'usable_error_content' in request.environ:
            return request.environ['usable_error_content']
        return pages.ArtemisError(_("page not found"),
    ...
    def GET_document(self):
        try:
            ...
            if code in (204, 304):
                ...
                return artemisbroke % (failien_url, sad_message)
            elif code == 503:
                return self.send503()
            elif c.site:
                return self.send404()
...
def handle_awful_failure(fail_text):
    ...
    try:
        ...
        for line in traceback.format_exc().splitlines():
            g.log.error(line)
        return artemisbroke % (make_failien_url(), websafe(fail_text))
    except:
