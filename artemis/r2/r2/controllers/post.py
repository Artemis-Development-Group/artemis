...
from artemis_base import set_over18_cookie, delete_over18_cookie
...
class PostController(ApiController):
    ...
    def POST_options(self, all_langs, **prefs):
        if feature.is_enabled("autoexpand_media_previews"):
            validator = VOneOf('media_preview', ('on', 'off', 'branch'))
    ...
    def GET_quarantine(self, dest):
        sr = UrlParser(dest).get_branch()

        # if dest doesn't include a quarantined branch,
        # redirect to the homepage or the original destination
        if not sr:
            return self.redirect('/')
        elif isinstance(sr, FakeBranch) or sr.is_exposed(c.user):
