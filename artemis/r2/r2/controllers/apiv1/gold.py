...
from r2.controllers.artemis_base import OAuth2OnlyController
...
class APIv1GoldController(OAuth2OnlyController):
    ...
    @require_oauth2_scope("cbranches")
    @validate(
        VUser(),
    ...
    def POST_gild(self, target):
        if not isinstance(target, (Comment, Link)):
            err = ArtemisError("NO_THING_ID")
            self.on_validation_error(err)

        if target.branch_slow.quarantine:
            err = ArtemisError("GILDING_NOT_ALLOWED")
            self.on_validation_error(err)
        VNotInTimeout().run(target=target, branch=target.branch_slow)

        self._gift_using_cbranches(
