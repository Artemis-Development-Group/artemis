...
from artemis_base import ArtemisController, MinimalController, require_https
from r2.lib.db import tdb_cassandra
from r2.lib.db.thing import NotFound
from r2.lib.pages import ArtemisError
...
class OAuth2FrontendController(ArtemisController):
    ...
    def POST_authorize(self, authorize, client, redirect_uri, scope, state,
        ...
        if (g.live_config.get('mobile_gild_first_login') and
            ...
            subject = 'Let there be gold! Artemis just sent you Artemis gold!'
            message = (
                "Thank you for using the Artemis mobile app!  As a thank you "
                "for logging in during launch week, you've been gifted %s of "
                "Artemis Gold.\n\n"
                "Artemis Gold is Artemis's premium membership program, which "
                "grants you: \n"
                "An ads-free experience in Artemis's mobile apps, and\n"
...
class OAuth2AccessController(MinimalController):
    ...
    def _get_client_auth(self):
        ...
        try:
            ...
            abort(401, headers=[("WWW-Authenticate", 'Basic realm="artemis"')])
    ...
    def POST_access_token(self, grant_type):
        ...
        **redirect_uri** must exactly match the value that was used in the call
        ...
        See artemis's [OAuth2 wiki](https://github.com/artemis/artemis/wiki/OAuth2) for
        more information.
