...
from r2.controllers.artemis_base import ArtemisController, abort_with_error
from r2.controllers.oauth2 import require_oauth2_scope
from r2.models.account import Account
from r2.models.branch import (
    FakeBranch,
    Branch,
    LabeledMulti,
...
multi_sr_data_json_spec = VValidatedJSON.Object({
    'name': VBranchName('name', allow_language_srs=True),
})
...
multi_json_spec = VValidatedJSON.PartialObject({
    ...
    'branches': VValidatedJSON.ArrayOf(multi_sr_data_json_spec),
})
...
class MultiApiController(ArtemisController):
    ...
    def GET_multi(self, multi, expand_srs):
        """Fetch a multi's data and branch list by name."""
    ...
    def _check_new_multi_path(self, path_info):
        if path_info['owner'].lower() != c.user.name.lower():
            raise ArtemisError('MULTI_CANNOT_EDIT', code=403,
                              fields='multipath')
        return c.user

    def _add_multi_srs(self, multi, sr_datas):
        srs = Branch._by_name(sr_data['name'] for sr_data in sr_datas)

        for sr in srs.itervalues():
            if isinstance(sr, FakeBranch):
                raise ArtemisError('MULTI_SPECIAL_BRANCH',
        ...
        for sr_data in sr_datas:
            try:
                ...
                raise ArtemisError('BRANCH_NOEXIST', code=400)
            else:
        ...
        except TooManyBranchesError as e:
            raise ArtemisError('MULTI_TOO_MANY_BRANCHS', code=409)

        return sr_props

    def _write_multi_data(self, multi, data):
        srs = data.pop('branches', None)
        if srs is not None:
            multi.clear_srs()
            try:
    ...
    def POST_multi(self, path_info, data):
        ...
        if not path_info:
            raise ArtemisError('BAD_MULTI_PATH', code=400)
        ...
        try:
            ...
            raise ArtemisError('MULTI_EXISTS', code=409, fields='multipath')
    ...
    def _copy_multi(self, from_multi, to_path_info, rename=False):
        ...
        if rename and from_multi.owner != to_owner:
            raise ArtemisError('MULTI_CANNOT_EDIT', code=400)
    ...
    @validate(
        ...
        to_path_info=VMultiPath("to", required=False,
            docs={"to": "destination multiartemis url path"},
        ),
        display_name=VLength("display_name", max_length=MAX_DISP_NAME,
                             empty_error=None),
    )
    ...
    def _get_multi_branch(self, multi, sr):
    ...
    def GET_multi_branch(self, multi, sr):
        """Get data about a branch in a multi."""
        return self._get_multi_branch(multi, sr)
    ...
    @validate(
        ...
        VModhash(),
        multi=VMultiByPath("multipath", require_edit=True),
        sr_name=VBranchName('srname', allow_language_srs=True),
        data=VValidatedJSON("model", multi_sr_data_json_spec),
    )
    @api_doc(api_section.multis, extends=GET_multi_branch)
    def PUT_multi_branch(self, multi, sr_name, data):
        """Add a branch to a multi."""
    ...
    @api_doc(api_section.multis, extends=GET_multi_branch)
    def DELETE_multi_branch(self, multi, sr):
        """Remove a branch from a multi."""
        multi.del_srs(sr)
        multi._commit()
