...
from artemis_base import ArtemisController
from r2.controllers.oauth2 import require_oauth2_scope
from r2.lib.utils import url_links_builder
from artemis_base import paginated_listing
...
from r2.models.branch import Branch
...
from artemis_base import base_listing
...
from r2.lib.errors import artemis_http_error
from r2.lib.automoderator import Ruleset

import json

page_descriptions = {
    "config/stylesheet": _("This page is the branch stylesheet, changes here apply to the branch css"),
    "config/submit_text": _("The contents of this page appear on the submit page"),
    "config/sidebar": _("The contents of this page appear on the branch sidebar"),
    "config/description": _("The contents of this page appear in the public branch description and when the user does not have access to the branch"),
    "config/automoderator": _("This page is used to configure AutoModerator for the branch, please see [the full documentation](/wiki/automoderator/full-documentation) for information"),
}
...
RENDERERS_BY_PAGE = {
    ...
    "config/description": "artemis",
    "config/sidebar": "artemis",
    "config/stylesheet": "stylesheet",
    "config/submit_text": "artemis",
    "toolbox": "rawcode",
    "usernotes": "rawcode",
}

class WikiController(ArtemisController):
    ...
    def GET_wiki_create(self, wp, page):
        ...
        if wp[0]:
            ...
            if error['reason'] == 'PAGE_NAME_LENGTH':
                ...
                error_msg = _("this page is a special page, please go into the branch settings and save the field once to create this special page")
    ...
    def GET_wiki_recent(self, num, after, reverse, count):
        """Retrieve a list of recently changed wiki pages in this branch"""
    ...
    def GET_wiki_listing(self):
        """Retrieve a list of wiki pages in this branch"""
    ...
    def on_validation_error(self, error):
        ArtemisController.on_validation_error(self, error)
        if error.code:
            self.handle_error(error.code, error.name)

    def handle_error(self, code, reason=None, **data):
        abort(artemis_http_error(code, reason, **data))

    def pre(self):
        ArtemisController.pre(self)
    ...
    def POST_wiki_edit(self, pageandprevious, content, page_name, reason):
        ...
        try:
            ...
            if page.name == "config/stylesheet":
                ...
                # continue storing the special pages as data attributes on the branch
                # object. TODO: change this to minimize branch get sizes.
