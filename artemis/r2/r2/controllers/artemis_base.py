...
def set_branch():
    ...
    sr_name = request.environ.get("branch", request.params.get('r'))
    ...
    if not sr_name:
        ...
        if cname:
            sr = Branch._by_domain(cname) or Frontpage
            domain = g.domain
            if g.domain_prefix:
        ...
        name_filter = lambda name: Branch.is_valid_name(name,
            allow_language_srs=True)
        sr_names = filter(name_filter, sr_name.split('+'))
        srs = Branch._by_name(sr_names, stale=can_stale).values()
        if All in srs:
            c.site = All
        elif Friends in srs:
            c.site = Friends
        else:
            srs = [sr for sr in srs if not isinstance(sr, FakeBranch)]
            if len(srs) == 1:
                c.site = srs[0]
            elif srs:
        ...
        if base_sr == All:
            ...
            path = "/branches/search?q=%s" % sr_name
            abort(302, location=BaseController.format_output_url(path))
    else:
        try:
            c.site = Branch._by_name(sr_name, stale=can_stale)
        except NotFound:
            if Branch.is_valid_name(sr_name):
                path = "/branches/search?q=%s" % sr_name
                abort(302, location=BaseController.format_output_url(path))
            elif not c.error_page and not request.path.startswith("/api/login/") :
                abort(404)

    #if we didn't find a branch, check for a domain listing
    ...
    if "multipath" in routes_dict or ("m" in request.GET and is_api()):
        ...
        if multi_ids is not None:
            ...
            if not multis:
                ...
                c.site = MultiArtemis(multiurl, srs)
...
def set_content_type():
    ...
    if e.has_key('extension'):
        ...
        if ext in ("mobile", "m") and not request.GET.get("keep_extension"):
            try:
                if request.cookies['artemis_mobility'] == "compact":
                    c.extension = "compact"
        ...
        if ext in ("mobile", "m", "compact"):
            if request.GET.get("keep_extension"):
                c.cookies['artemis_mobility'] = Cookie(ext, expires=NEVER)
...
def set_iface_lang():
    host_lang = request.environ.get('artemis-prefer-lang')
...
class MinimalController(BaseController):
    ...
    def run_sitewide_ratelimits(self):
        ...
        if type_ in ("cdn", "web", "none"):
            ...
            # * Web requests (HTML)
            # * CDN requests (logged out via www.Artemis)
            return
    ...
    def pre(self):
        ...
        c.domain_prefix = request.environ.get("artemis-domain-prefix",
                                              g.domain_prefix)
        ...
        hooks.get_hook("artemis.request.minimal_begin").call()
    ...
    def post(self):
        ...
        if c.user_is_loggedin or would_poison:
            ...
            # Normally you'd prefer `no-store`, but many of artemis's
        ...
        if not isinstance(c.site, FakeBranch) and not g.disallow_db_writes:
            if c.user_is_loggedin:
        ...
        hooks.get_hook("artemis.request.end").call()
...
class OAuth2ResourceController(MinimalController):
    ...
    def _auth_error(self, code, error):
        abort(code, headers=[("WWW-Authenticate", 'Bearer realm="artemis", error="%s"' % error)])
...
class ArtemisController(OAuth2ResourceController):
    ...
    def pre(self):
        ...
        # looking up the multiartemis requires c.user.
        set_multiartemis()
        ...
        if not isinstance(c.site, FakeBranch):
            request.environ['ARTEMIS_NAME'] = c.site.name

        # random artemis trickery
        if c.site == Random:
            c.site = Branch.random_artemis(user=c.user)
            ...
            c.site = Branch.random_subscription(c.user)
            site_path = c.site.path.strip('/')
            path = '/' + site_path + request.path_qs
            abort(302, location=self.format_output_url(path))
        elif c.site == RandomNSFW:
            c.site = Branch.random_artemis(over18=True, user=c.user)
            site_path = c.site.path.strip('/')
            path = '/' + site_path + request.path_qs
            abort(302, location=self.format_output_url(path))

        if not request.path.startswith("/api/login/"):
            # is the branch banned?
        ...
        has_style_override = (c.user.pref_default_theme_sr and
                ...
                Branch._by_name(c.user.pref_default_theme_sr).can_view(c.user))
        sr_stylesheet_enabled = c.user.use_branch_style(c.site)
        ...
        hooks.get_hook("artemis.request.begin").call()
    ...
    def post(self):
        ...
        if g.tracker_url and request.method.upper() == "GET" and is_api():
            ...
            response.headers["X-Artemis-Tracking"] = tracking_url
    ...
    def search_fail(self, exception):
        errpage = pages.ArtemisError(_("search failed"),
