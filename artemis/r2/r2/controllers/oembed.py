...
from r2.controllers.artemis_base import MinimalController
...
EMBEDLY_SCRIPT = 'https://embed.artemismedia.com/widgets/platform.js'
SCRIPT_TEMPLATE = '<script async src="%(embedly_script)s" charset="UTF-8"></script>'
POST_EMBED_TEMPLATE = """
    <blockquote class="artemis-card" %(live_data_attr)s>
...
def _oembed_post(thing, **embed_options):
    ...
    if (not can_view_link_comments(thing) or
            branch.type in Branch.private_types):
    ...
    html = format_html(POST_EMBED_TEMPLATE,
                       ...
                       branch_url=make_url_https(branch.path),
