...
from r2.models import Link, Branch
...
def get_recommendations(srs,
                        count=10,
                        source=SRC_MULTIARTEMISS,
                        to_omit=None,
                        match_set=True,
                        over18=False):
    """Return branches recommended if you like the given branches.

    Args:
    - srs is one Branch object or a list of Branches
    - count is total number of results to return
    - source is a prefix telling which set of recommendations to use
    - to_omit is a single or list of branch id36s that should not be
    ...
    # always check for private branches at runtime since type might change
    rec_srs = Branch._byID36(rec_id36s, return_dict=False)
...
def get_recommended_content(prefs, src, settings):
    """Get a mix of content from branches recommended for someone with
    ...
    num_discovery = 2  # how many discovery-related branches to mix in
    ...
    default_srid36s = [to36(srid) for srid in Branch.default_branches()]
    ...
    if settings.discovery:
        # get links from branches dedicated to discovery
    ...
    # make sure branches aren't repeated
    seen_srs = set()
    recs = []
    for r in all_recs:
...
def get_rising_items(omit_sr_ids, count=4):
    ...
    rising_links = Link._by_fullname(link_fullnames_to_show,
                                     return_dict=False,
                                     data=True)
    rising_items = [ExploreItem(TYPE_RISING, 'ris', Branch._byID(l.sr_id), l)
...
def get_comment_items(srs, src, count=4):
    ...
    srs = Branch._byID([com.sr_id for com in top_comments])
...
def get_discovery_srid36s():
    ...
    srs = Branch._by_name(g.live_config['discovery_srs'])
...
def is_visible(sr):
    ...
    return (
        sr.type not in Branch.private_types and
        not sr._spam and
        sr.discoverable
    )
...
class SRRecommendation(tdb_cassandra.View):
    ...
    # N runs didn't generate recommendations for a given branch
