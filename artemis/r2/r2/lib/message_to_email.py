...
def get_reply_to_address(message):
    ...
    sr = Branch._byID(message.sr_id, data=True)
    return "r/{branch} mail <{reply_id}@{domain}>".format(
        branch=sr.name, reply_id=reply_id, domain=g.modmail_email_domain)
...
def get_message_subject(message):
    ...
    return u"[r/{branch} mail]: {subject}".format(
        branch=sr.name, subject=_force_unicode(conversation_subject))
...
def get_system_from_address(sr):
    return "r/{branch} mail <{sender_email}>".format(
        branch=sr.name, sender_email=g.modmail_system_email)
...
def send_modmail_email(message):
    ...
    if message.from_sr and not message.first_message:
        # this is a message from the branch to a user. add some text that
        # shows the recipient
        recipient = Account._byID(message.to_id, data=True)
        sender_text = ("This message was sent from r/{branch} to "
            "u/{user}").format(branch=sr.name, user=recipient.name)
    else:
    ...
    reply_footer = ("\n\n-\n{sender_text}\n\n"
        "Reply to this email directly or view it on artemis: {link}")
...
def process_modmail_email():
    ...
    def process_message(msg):
        ...
        if msg_dict["event"] == "new_message":
            ...
            branch_id36 = msg_dict["branch_id36"]
            sr = Branch._byID36(branch_id36, data=True)
