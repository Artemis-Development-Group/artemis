...
#
# All portions of the code written by reddit are Copyright (c) 2006-2016 artemis
...
def register_detector(cls):
    """Collector of all the artemis detectors."""
    detectorshub.register(cls())
    return cls


class ArtemisDetectorBase(DetectorBase):
    agent_string = None
    version_string = '(\.?\d+)*'
    ...
    def detect(self, agent, result):
        detected = super(ArtemisDetectorBase, self).detect(agent, result)
...
class ArtemisBrowser(ArtemisDetectorBase, Browser):
    """Base class for all artemis specific browsers."""
...
@register_detector
class ArtemisIsFunDetector(ArtemisBrowser):
    is_app = True
    look_for = 'artemis is fun'
    name = 'artemis is fun'
...
@register_detector
class ArtemisAndroidDetector(ArtemisBrowser):
    is_app = True
    look_for = 'ArtemisAndroid'
    name = 'Artemis: The Official App'
    agent_string = '{look_for} (?P<version>{version_string})$'


@register_detector
class ArtemisIOSDetector(ArtemisBrowser):
    is_app = True
    look_for = 'Artemis'
    name = 'artemis iOS'
...
@register_detector
class AlienBlueDetector(ArtemisBrowser):
    is_app = True
    look_for = 'AlienBlue'
    name = 'Alien Blue'
    agent_string = (
...
@register_detector
class RelayForArtemisDetector(ArtemisBrowser):
    is_app = True
    look_for = 'Relay by /u/DBrady'
    name = 'relay for artemis'
    agent_string = '{look_for} v(?P<version>{version_string})'


@register_detector
class ArtemisSyncDetector(ArtemisBrowser):
    is_app = True
    look_for = 'artemis_sync'
    name = 'Sync for artemis'
    agent_string = (
...
@register_detector
class NarwhalForArtemisDetector(ArtemisBrowser):
    is_app = True
    look_for = 'narwhal'
    name = 'narwhal for artemis'
    agent_string = '{look_for}-(?P<platform>.*?)\/\d+ by det0ur'


@register_detector
class McArtemisDetector(ArtemisBrowser):
    is_app = True
    look_for = 'McArtemis'
    name = 'McArtemis'
    agent_string = '{look_for} - Artemis Client for (?P<platform>.*?)$'


@register_detector
class ReaditDetector(ArtemisBrowser):
    look_for = 'Readit'
    name = 'Readit'
...
@register_detector
class BaconReaderDetector(ArtemisBrowser):
    is_app = True
