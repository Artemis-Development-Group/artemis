...
import snudown
import unidecode
from r2.lib.utils import artemis_agent_parser
...
class UrlParser(object):
    ...
    The class also contains artemis-specific functions for setting,
    checking, and getting a path's branch.
    """
    ...
    def switch_subdomain_by_extension(self, extension=None):
        ...
        This should only be used on artemis URLs.
        ...
        * extension: the template extension to which the middleware hints when
        ...
        >>> u.unparse()
        'http://i.reddit.com/r/artemisdev'
    ...
    def path_has_branch(self):
        """
        utility method for checking if the path starts with a
        branch specifier (namely /r/ or /branches/).
        """
        return self.path.startswith(('/r/', '/branches/', '/branches/'))

    def get_branch(self):
        """checks if the current url refers to a branch and returns
        that branch object.  The cases here are:

          * the hostname is unset or is g.domain, in which case it
            looks for /r/XXXX or /branches.  The default in this case
            is Default.
          * the hostname is a cname to a known branch.

        On failure to find a branch, returns None.
        """
        from r2.models import Branch, NotFound, DefaultSR
        try:
            if (not self.hostname or
                    is_subdomain(self.hostname, g.domain) or
                    self.hostname.startswith(g.domain)):
                if self.path.startswith('/r/'):
                    return Branch._by_name(self.path.split('/')[2])
                else:
                    return DefaultSR()
            elif self.hostname:
                return Branch._by_domain(self.hostname)
        except NotFound:
            pass
        return None
    ...
    def is_web_safe_url(self):
        ...
        # Credentials in the netloc? Not on artemis!
    ...
    def is_artemis_url(self, branch=None):
        """utility method for seeing if the url is associated with
        artemis as we don't necessarily want to mangle non-artemis
        domains

        returns true only if hostname is nonexistant, a subdomain of
        g.domain, or a subdomain of the provided branch's cname.
        """

        valid_subdomain = (
            not self.hostname or
            is_subdomain(self.hostname, g.domain) or
            (branch and branch.domain and
                is_subdomain(self.hostname, branch.domain))
        )
    ...
    def path_add_branch(self, branch):
        """
        Adds the branch's path to the path if another branch's
        prefix is not already present.
        """
        if not (self.path_has_branch()
                or self.path.startswith(branch.user_path)):
            self.path = (branch.user_path + self.path)
        return self
...
def url_to_thing(url):
    """Given a artemis URL, return the Thing to which it associates.
    ...
    """
    from r2.models import Comment, Link, Message, NotFound, Branch, Thing
    from r2.config.middleware import BranchMiddleware
    sr_pattern = BranchMiddleware.sr_pattern
    ...
    if not path or path == '/':
        ...
        try:
            return Branch._by_name(sr_name, data=True)
        except NotFound:
            return None
...
def fix_if_broken(thing, delete = True, fudge_links = False):
    from r2.models import Link, Comment, Branch, Message

    # the minimum set of attributes that are required
    attrs = dict((cls, cls._essentials)
                 for cls
                 in (Link, Comment, Branch, Message))
...
def url_links_builder(url, exclude=None, num=None, after=None, reverse=None,
    ...
    from r2.models import IDBuilder, Link, NotFound, Branch
    ...
    if public_srs_only and not c.user_is_admin:
        branches = Branch._byID([link.sr_id for link in links], data=True)
        links = [link for link in links
                 if branches[link.sr_id].type not in Branch.private_types]
        ...
        return (not link._spam or
                (c.user_is_loggedin and
                    (link.author_id == c.user._id or
                        c.user_is_admin or
                        link.branch.is_moderator(c.user))))
