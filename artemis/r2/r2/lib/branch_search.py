...
class BranchesByPartialName(tdb_cassandra.View):
    _use_db = True
    _value_type = 'pickle'
...
def load_all_branches():
    ...
    q = Branch._query(Branch.c.type == 'public',
                         Branch.c._spam == False,
                         Branch.c._downs > 1,
    ...
    for name_prefix, branches in query_cache.iteritems():
        BranchesByPartialName._set_values(name_prefix, {'tups': branches})

def search_branches(query, include_over_18=True):
    query = str(query.lower())

    try:
        result = BranchesByPartialName._byID(query)
...
def popular_searches(include_over_18=True):
    top_branches = Branch._query(Branch.c.type == 'public',
                                   sort = desc('_downs'),
                                   limit = 100,
                                   data = True)
    top_searches = {}
    for sr in top_branches:
        if sr.quarantine:
            continue
        ...
        for i in xrange(min(len(name), 3)):
            query = name[:i + 1]
            r = search_branches(query, include_over_18)
