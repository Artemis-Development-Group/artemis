...
class EventQueue(object):
    ...
    def vote_event(self, vote):
        ...
        event.add_branch_fields(vote.thing.branch_slow)
    ...
    def submit_event(self, new_post, request=None, context=None):
        ...
        event.add_branch_fields(new_post.branch_slow)
    ...
    def comment_event(self, new_comment, request=None, context=None):
        ...
        event.add_branch_fields(new_comment.branch_slow)
    ...
    @squelch_exceptions
    def muted_forbidden_event(self, details_text, branch=None,
        ...
        details_text: "muted" if a muted user is trying to message the
            branch or "muted mod" if the branch mod is attempting
            to message the muted user
        branch: The Branch of the mod messaging the muted user
        parent_message: Message that is being responded to
        target: The intended recipient (Branch or Account)
        request, context: Should be pylons.request & pylons.c respectively;
        ...
        event.add_branch_fields(branch)
    ...
    def timeout_forbidden_event(self, action_name, details_text,
            target=None, target_fullname=None, branch=None,
        ...
        branch: The Branch the action was taken in. If target is of the
            type Branch, then this won't be passed in
        request, context: Should be pylons.request & pylons.c respectively;
        ...
        if not action_name:
            ...
            # set or unset for contest mode and branch sticky
            elif action_name in ("set_contest_mode", "set_branch_sticky"):
        ...
        from r2.models import Comment, Link, Branch
        if not branch:
            if isinstance(context.site, Branch):
                branch = context.site
            elif isinstance(target, (Comment, Link)):
                branch = target.branch_slow
    ...
    def mod_event(self, modaction, branch, mod, target=None,
            request=None, context=None):
        """Create a 'mod' event for event-collector.

        modaction: An r2.models.ModAction object
        branch: The Branch the mod action is being performed in
    ...
    def report_event(self, reason=None, details_text=None,
            branch=None, target=None, request=None, context=None,
        ...
        branch: The Branch the action is being performed in
        target: The Thing the action was applied to
        request, context: Should be pylons.request & pylons.c respectively

        """
        from r2.models.rules import OLD_SITEWIDE_RULES, SITEWIDE_RULES, BranchRules
        ...
        if reason in OLD_SITEWIDE_RULES or reason in SITEWIDE_RULES:
            ...
            if branch and BranchRules.get_rule(branch, reason):
                process_notes = "BRANCH_RULES"
            else:
    ...
    def quarantine_event(self, event_type, branch,
            request=None, context=None):
...
class Event(object):
    ...
    def add_target_fields(self, target):
        ...
        # If the target is an Account or Branch (or has a "name" attr),
        # add the target_name
    ...
    def add_branch_fields(self, branch):
        if not branch:
            return

        self.add("sr_id", branch._id)
        self.add("sr_name", branch.name)
