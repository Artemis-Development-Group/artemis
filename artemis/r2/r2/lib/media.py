...
def _get_scrape_url(link):
    if not link.is_self:
        sr_name = link.branch_slow.name
        if not feature.is_enabled("imgur_gif_conversion", branch=sr_name):
            return link.url
...
def _set_media(link, force=False, **kwargs):
    ...
    # Do not process thumbnails for quarantined branches
    if sr.quarantine:
        return
    ...
    youtube_scraper = feature.is_enabled("youtube_scraper", branch=sr.name)
