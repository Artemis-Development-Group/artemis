...
class CachedResults(object):
    ...
    def can_insert(self):
        """True if a new item can just be inserted rather than
         ...
         # the top hottest items in X some branch where the query
...
def _get_links(sr_id, sort, time):
    """General link query for a branch."""
...
def moderated_srids(sr, user):
    if isinstance(sr, (ModContribSR, MultiArtemis)):
        srs = Branch._byID(sr.sr_ids, return_dict=False)
        if user:
...
def _get_sr_comments(sr_id):
    """the branch /r/foo/comments page"""
...
cached_srrel_query = cached_query(BranchQueryCache, filter_thing2)
...
@cached_srrel_query
def get_branch_messages(sr):
    return rel_query(ModeratorInbox, sr, 'inbox')

@cached_srrel_query
def get_unread_branch_messages(sr):
    return rel_query(ModeratorInbox, sr, 'inbox',
                          filters = [ModeratorInbox.c.new == True])

def get_unread_branch_messages_multi(srs):
    if not srs:
        return []
    queries = [get_unread_branch_messages(sr) for sr in srs]
    return MergedCachedQuery(queries)
...
def _promoted_link_query(user_id, status):
    ...
    q = Link._query(Link.c.sr_id == Branch.get_promote_srid(),
                    Link.c._spam == (True, False),
...
@cached_query(BranchQueryCache, filter_fn=filter_thing)
...
def new_branch(sr):
    ...
    amqp.add_item('new_branch', sr._fullname)
...
def new_message(message, inbox_rels, add_to_sent=True, update_modmail=True):
    ...
    with CachedQueryMutator() as m:
        ...
        for inbox_rel in tup(inbox_rels):
            ...
            if isinstance(inbox_rel, ModeratorInbox):
                m.insert(get_branch_messages(to), [inbox_rel])
    ...
    if update_modmail:
        mod_perms = message.branch_slow.moderators_with_perms()
...
def update_unread_sr_queries(inbox_rels, insert=True, mutator=None):
    ...
    for inbox_rel in inbox_rels:
        ...
        query = get_unread_branch_messages(sr)
...
def unread_handler(things, user, unread):
    ...
    comments = []
    messages = []
    # Group things by branch or type
    for thing in things:
    ...
    if sr_messages:
        mod_srs = Branch.reverse_moderator_ids(user)
        srs = Branch._byID(sr_messages.keys())
    else:
        mod_srs = []
...
def _by_srid(things, srs=True):
    """Takes a list of things and returns them in a dict separated by
       sr_id, in addition to the looked-up branches"""
    ret = {}
    ...
    if srs:
        _srs = Branch._byID(ret.keys(), return_dict=True) if ret else {}
        return ret, _srs
    else:
        return ret
...
def ban(things, filtered=True):
    ...
    for sr_id, sr_things in by_srid.iteritems():
        ...
        for item in sr_things:
            # don't add posts by banned users if branch prefs exclude them
            add_to_modqueue = (filtered and
                       not (item.branch_slow.exclude_banned_modqueue and
...
def new_report(thing, report_rel):
    ...
    # report queue as well as the per-branch one
...
def add_all_srs():
    """Recalculates every listing query for every branch. Very,
       very slow."""
    q = Branch._query(sort = asc('_date'))
