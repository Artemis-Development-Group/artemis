...
def branch_messages(sr, update = False):
    key = sr_messages_key(sr._id)
    trees = g.permacache.get(key)
    if not trees or update:
        trees = branch_messages_nocache(sr)
        g.permacache.set(key, trees)
    return trees
...
def moderator_messages(sr_ids):
    ...
    def multi_load_tree(sr_ids):
        res = {}
        for sr_id in sr_ids:
            trees = branch_messages_nocache(srs[sr_id])
            if trees:
                res[sr_id] = trees
        return res
...
def branch_messages_nocache(sr):
    """
    Just like user_messages, but avoiding the cache
    """
    from r2.lib.db import queries
    inbox = queries.get_branch_messages(sr)
