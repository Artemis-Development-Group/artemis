...
SAFE_PROPERTIES = {
    ...
    # use branch stylesheets. see explanation here:
    # https://github.com/artemis/artemis/pull/1058#issuecomment-76466180
    # "filter",
    "flex",
    "flex-align",
    "flex-basis",
...
MAX_SIZE_KIB = 100
BRANCH_IMAGE_URL_PLACEHOLDER = re.compile(r"\A%%([a-zA-Z0-9\-]+)%%\Z")
...
class StylesheetValidator(object):
    ...
    def validate_url(self, url_node):
        m = BRANCH_IMAGE_URL_PLACEHOLDER.match(url_node.value)
        if not m:
...
def validate_css(stylesheet, images):
    ...
    images is a mapping of branch image names to their URLs.  The
