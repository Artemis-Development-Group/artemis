...
from r2.models import Account, Comment, Link, Branch
...
def add_to_branch_query_q(link):
    if g.shard_branch_query_queues:
        branch_shard = link.sr_id % 10
        queue_name = "branch_query_%s_q" % branch_shard
    else:
        queue_name = "branch_query_q"
    amqp.add_item(queue_name, link._fullname)


def consume_branch_query_queue(qname="branch_query_q", limit=1000):
    @g.stats.amqp_processor(qname)
    def process_message(msgs, chan):
        """Update get_links(), the Links by Branch precomputed query.

        get_links() is a CachedResult which is stored in permacache. To
        update these objects we need to do a read-modify-write which requires
        obtaining a lock. Sharding these updates by branch allows us to run
        multiple consumers (but ideally just one per shard) to avoid lock
        contention.
        ...
        srs_by_id = Branch._byID(links_by_sr_id.keys(), stale=True)

        for sr_id, links in links_by_sr_id.iteritems():
            with g.stats.get_timer("link_vote_processor.branch_queries"):
                sr = srs_by_id[sr_id]
                add_queries(
