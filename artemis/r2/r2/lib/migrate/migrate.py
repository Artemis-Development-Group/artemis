...
"""
One-time use functions to migrate from one artemis-version to another
"""
from r2.lib.promote import *

def add_allow_top_to_srs():
    "Add the allow_top property to all stored branches"
    from r2.models import Branch
    from r2.lib.db.operators import desc
    from r2.lib.utils import fetch_things2

    q = Branch._query(Branch.c._spam == (True,False),
                         sort = desc('_date'))
...
def subscribe_to_blog_and_annoucements(filename):
    ...
    from r2.models import Account, Branch

    r_blog = Branch._by_name("blog")
    r_announcements = Branch._by_name("announcements")
...
def pushup_permacache(verbosity=1000):
    ...
    from r2.models import Link, Branch, Account
    from r2.lib.db.operators import desc
    ...
    from r2.lib.branch_search import load_all_branches
    from r2.lib.db import queries
    ...
    def gen_keys():
        ...
        sr_q = Branch._query(Branch.c._spam == (True, False),
                                sort=desc('_date'),
                                )
        for sr in fetch_things2(sr_q, verbosity):
            ...
            yield queries.get_branch_messages(sr).iden
            yield queries.get_unread_branch_messages(sr).iden
...
def port_cassaurls(after_id=None, estimate=15231317):
    from r2.models import Link, LinksByUrlAndBranch
