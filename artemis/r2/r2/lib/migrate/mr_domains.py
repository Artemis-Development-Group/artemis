...
"""
Generate the data for the listings for the time-based Branch
...
cd ~/artemis/r2
time psql -F"\t" -A -t -d newartemis -U $USER -h $LINKDBHOST \
     -c "\\copy (select t.thing_id, 'thing', 'link',
                        t.ups, t.downs, t.deleted, t.spam, extract(epoch from t.date)
                   from artemis_thing_link t
                  where not t.spam and not t.deleted
                  )
                  to 'artemis_thing_link.dump'"
time psql -F"\t" -A -t -d newartemis -U $USER -h $LINKDBHOST \
     -c "\\copy (select d.thing_id, 'data', 'link',
                        d.key, d.value
                   from artemis_data_link d
                  where d.key = 'url' ) to 'artemis_data_link.dump'"
cat artemis_data_link.dump artemis_thing_link.dump | sort -T. -S200m | paster --plugin=r2 run $INI r2/lib/migrate/mr_domains.py -c "join_links()" > links.joined
...
from r2.models import Account, Branch, Link
