...
## links
time psql -F"\t" -A -t -d newartemis -U ri -h $LINKDBHOST \
     -c "\\copy (select t.thing_id, 'thing', 'link',
                        t.ups, t.downs, t.deleted, t.spam, extract(epoch from t.date)
                   from artemis_thing_link t) to 'artemis_thing_link.dump'"
time psql -F"\t" -A -t -d newartemis -U ri -h $LINKDBHOST \
     -c "\\copy (select d.thing_id, 'data', 'link',
                        d.key, d.value
                   from artemis_data_link d
                  where d.key = 'author_id' or d.key = 'sr_id') to 'artemis_data_link.dump'"
pv artemis_data_link.dump artemis_thing_link.dump | sort -T. -S200m | ./mr_permacache "join_links()" > links.joined
pv links.joined | ./mr_permacache "link_listings()" | sort -T. -S200m > links.listings

## comments
psql -F"\t" -A -t -d newartemis -U ri -h $COMMENTDBHOST \
     -c "\\copy (select t.thing_id, 'thing', 'comment',
                        t.ups, t.downs, t.deleted, t.spam, extract(epoch from t.date)
                   from artemis_thing_comment t) to 'artemis_thing_comment.dump'"
psql -F"\t" -A -t -d newartemis -U ri -h $COMMENTDBHOST \
     -c "\\copy (select d.thing_id, 'data', 'comment',
                        d.key, d.value
                   from artemis_data_comment d
                  where d.key = 'author_id') to 'artemis_data_comment.dump'"
cat artemis_data_comment.dump artemis_thing_comment.dump | sort -T. -S200m | ./mr_permacache "join_comments()" > comments.joined
cat links.joined | ./mr_permacache "comment_listings()" | sort -T. -S200m > comments.listings

## linkvotes
psql -F"\t" -A -t -d newartemis -U ri -h $VOTEDBHOST \
     -c "\\copy (select r.rel_id, 'vote_account_link',
                        r.thing1_id, r.thing2_id, r.name, extract(epoch from r.date)
                   from artemis_rel_vote_account_link r) to 'artemis_linkvote.dump'"
pv artemis_linkvote.dump | ./mr_permacache "linkvote_listings()" | sort -T. -S200m > linkvotes.listings

#savehide
psql -F"\t" -A -t -d newartemis -U ri -h $SAVEHIDEDBHOST \
     -c "\\copy (select r.rel_id, 'savehide',
                        r.thing1_id, r.thing2_id, r.name, extract(epoch from r.date)
                   from artemis_rel_savehide r) to 'artemis_savehide.dump'"
pv artemis_savehide.dump | ./mr_permacache "savehide_listings()" | sort -T. -S200m > savehide.listings
...
def store_keys(key, maxes):
    ...
    # individual lookups for accounts and branches
    ...
    if key.startswith('user-'):
        ...
        q = queries.get_links(Branch._byID(sr_id), sort, time)
