...
def notify_user_added(rel_type, author, user, target):
    ...
    if "pm" in msgs and author != user:
        ...
        if rel_type in ("moderator_invite", "contributor"):
            # send the message from the branch
...
def send_mod_removal_message(branch, mod, user):
    sr_name = "/r/" + branch.name
    u_name = "/u/" + user.name
    subject = "%(user)s has been removed as a moderator from %(branch)s"
    message = (
        "%(user)s: You have been removed as a moderator from %(branch)s.  "
        "If you have a question regarding your removal, you can "
        "contact the moderator team for %(branch)s by replying to this "
        "message."
    )
    subject %= {"branch": sr_name, "user": u_name}
    message %= {"branch": sr_name, "user": user.name}
...
def send_ban_message(branch, mod, user, note=None, days=None, new=True):
    sr_name = "/r/" + branch.name
    if days:
        subject = "You've been temporarily banned from participating in %(branch)s"
        message = ("You have been temporarily banned from participating in "
            "%(branch)s. This ban will last for %(duration)s days. ")
    else:
        subject = "You've been banned from participating in %(branch)s"
        message = "You have been banned from participating in %(branch)s. "

    message += ("You can still view and subscribe to %(branch)s, but you "
                "won't be able to post or comment.")

    if not new:
        subject = "Your ban from %(branch)s has changed"

    subject %= {"branch": sr_name}
    message %= {"branch": sr_name, "duration": days}
    ...
    message += "\n\n" + ("If you have a question regarding your ban, you can "
        "contact the moderator team for %(branch)s by replying to this "
        "message.") % {"branch": sr_name}

    message += "\n\n" + ("**Reminder from the Artemis staff**: If you use "
        "another account to circumvent this branch ban, that will be "
        "considered a violation of [the Content Policy](/help/contentpolicy#section_prohibited_behavior) "
        "and can result in your account being [suspended](https://artemis.zendesk.com/hc/en-us/articles/205687686) "
        "from the site as a whole.")

    item, inbox_rel = Message._new(
        mod, user, subject, message, request.ip, sr=branch, from_sr=True,
        can_send_email=False)
    queries.new_message(item, inbox_rel, update_modmail=False)
