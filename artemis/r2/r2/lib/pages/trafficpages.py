...
###############################################################################
"""View models for the traffic statistic pages on artemis."""
...
from r2.lib.pages.pages import Artemis, TimeSeriesChart, TabbedPane
...
from r2.models.branch import Branch, _DefaultSR
...
class TrafficPage(Artemis):
    ...
    def __init__(self, content):
        Artemis.__init__(self, title=_("traffic stats"), content=content)
...
class ArtemisTraffic(Templated):
...
def make_branch_traffic_report(branches=None, num=None):
    """Return a report of branch traffic in the last full month.

    If given a list of branches, those branches will be put in the report
    otherwise the top branches by pageviews will be automatically chosen.

    """

    if branches:
        branch_summary = traffic.PageviewsByBranch.last_month(branches)
    else:
        branch_summary = traffic.PageviewsByBranch.top_last_month(num)

    report = []
    for srname, data in branch_summary:
        if srname == _DefaultSR.name:
            name = _("[frontpage]")
            url = None
        elif srname in Branch._specials:
            name = "[%s]" % srname
            url = None
        else:
            name = "/r/%s" % srname
...
class SitewideTraffic(ArtemisTraffic):
    """An overview of all traffic to the site."""
    def __init__(self):
        self.branch_summary = make_branch_traffic_report(num=250)
        ArtemisTraffic.__init__(self, g.domain)
...
class AdvertTrafficSummary(ArtemisTraffic):
    """An overview of traffic for all adverts on the site."""

    def __init__(self):
        ArtemisTraffic.__init__(self, _("adverts"))
    ...
    def get_sr_name(name):
        """Return the display name for a branch."""
...
class LanguageTraffic(ArtemisTraffic):
    def __init__(self, langcode):
        self.langcode = langcode
        name = LanguageTraffic.get_language_name(langcode)
        ArtemisTraffic.__init__(self, name)
...
class AdvertTraffic(ArtemisTraffic):
    def __init__(self, code):
        self.code = code
...
class BranchTraffic(ArtemisTraffic):
    def __init__(self):
        ArtemisTraffic.__init__(self, "/r/" + c.site.name)
    ...
    def get_dow_summary(self):
        return traffic.PageviewsByBranch.history("day", c.site.name)

    def get_data_for_interval(self, interval, columns):
        pageviews = traffic.PageviewsByBranch.history(interval, c.site.name)
        ...
        if interval == "day":
            ...
            sr_name = c.site.name
            subscriptions = traffic.SubscriptionsByBranch.history(interval,
                                                                     sr_name)
...
class BranchTrafficReport(Templated):
    def __init__(self):
        self.srs, self.invalid_srs, self.report = [], [], []

        self.textarea = request.params.get("branches")
        if self.textarea:
            requested_srs = [srname.strip()
                             for srname in self.textarea.splitlines()]
            branches = Branch._by_name(requested_srs)

            for srname in requested_srs:
                if srname in branches:
                    self.srs.append(srname)
                else:
                    self.invalid_srs.append(srname)

            if branches:
                self.report = make_branch_traffic_report(branches.values())

            param = urllib.quote(self.textarea)
            self.csv_url = "/traffic/branches/report.csv?branches=" + param
