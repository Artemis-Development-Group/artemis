...
def may_revise(sr, user, page=None):    
    ...
    if sr.is_wikibanned(user):
        # Users who are wiki banned in the branch may not contribute
        return False
    
    if sr.is_banned(user):
        # If the user is banned from the branch, do not allow them to contribute
        return False
    ...
    if not sr.wiki_can_submit(user):
        # If the user can not submit to the branch
        # They should not be able to contribute
        return False

    # Use global karma for the frontpage wiki
    karma_sr = sr if sr.wiki_use_branch_karma else None
