...
from r2.models.rules import MAX_RULES_PER_BRANCH
...
from r2.lib.errors import errors, ArtemisError, UserRequiredException
...
def can_view_link_comments(article):
    return (article.branch_slow.can_view(c.user) and
...
class VMarkdown(Validator):
    def __init__(self, param, renderer='artemis'):
...
class VMarkdownLength(VMarkdown):
    def __init__(self, param, renderer='artemis', max_length=10000,
...
class VBranchName(VRequired):
    ...
    def param_docs(self):
        return {
            self.param: "branch name",
        }


class VAvailableBranchName(VBranchName):
    def run(self, name):
        name = VBranchName.run(self, name)
        if name:
            try:
                a = Branch._by_name(name)
                return self.error(errors.BRANCH_EXISTS)
            except NotFound:
                return name
...
class VSRByName(Validator):
    ...
    def run(self, sr_name):
        if not sr_name:
            ...
            try:
                sr = Branch._by_name(sr_name)
                if self.return_srname:
                    return sr.name
                else:
                    return sr
            except NotFound:
                self.set_error(errors.BRANCH_NOEXIST, code=400)
...
class VSRByNames(Validator):
    """Returns a dict mapping branch names to branch objects.

    sr_names_csv - a comma delimited string of branch names
    required - if true (default) an empty branch name list is an error
    ...
    def run(self, sr_names_csv):
        if sr_names_csv:
            ...
            return Branch._by_name(sr_names)
        elif self.required:
            self.set_error(errors.BAD_SR_NAME, code=400)
        return {}
    ...
    def param_docs(self):
        return {
            self.param: "comma-delimited list of branch names",
        }


class VBranchTitle(Validator):
    def run(self, title):
        if not title:
...
class VAvailableBranchRuleName(Validator):
    ...
    def run(self, short_name):
        ...
        if BranchRules.get_rule(c.site, short_name):
            self.set_error(errors.SR_RULE_EXISTS)
        elif not self.updating:
            number_rules = len(BranchRules.get_rules(c.site))
            if number_rules >= MAX_RULES_PER_BRANCH:
                self.set_error(errors.SR_RULE_TOO_MANY)
                return None
        return short_name


class VBranchRule(Validator):
    def run(self, short_name):
        short_name = VLength(
            self.param,
            max_length=50,
        ...
        rule = BranchRules.get_rule(c.site, short_name)
        if not rule:
            self.set_error(errors.SR_RULE_DOESNT_EXIST)
        else:
            return rule
...
class VAccountByName(VRequired):
    ...
    def param_docs(self):
        return {self.param: "A valid, existing artemis username"}
...
class VNotInTimeout(Validator):
    def run(self, target_fullname=None, fatal=True, action_name=None,
            details_text="", target=None, branch=None):
        if c.user_is_loggedin and c.user.in_timeout:
            ...
            if fatal:
                request.environ['ARTEMIS_ERROR_NAME'] = 'IN_TIMEOUT'
...
class VGold(VUser):
    notes = "*Requires a subscription to [artemis gold](/gold/about)*"
    def run(self):
        VUser.run(self)
        if not c.user.gold:
            abort(403, 'forbidden')
        ...
        if c.user_is_loggedin:
            ...
            if item.author_id == c.user._id:
                if isinstance(item, Message) and c.user.employee:
                    return True
                branch = item.branch_slow

                if (how in ("yes", "no") and
                        branch.can_distinguish(c.user)):
                ...
                if can_distinguish:
                    ...
                    VNotInTimeout().run(target=item, branch=branch)
    ...
    def run(self, thing_name):
        if c.user_is_admin:
            ...
            if item.author_id == c.user._id:
                ...
                # comments
                branch = item.branch_slow
                if branch.can_distinguish(c.user):
                    can_alter = True
...
class VSubmitSR(Validator):
    ...
    def run(self, sr_name, link_type = None):
        if not sr_name:
            self.set_error(errors.BRANCH_REQUIRED)
            return None
        ...
        try:
            ...
            self.set_error(errors.BRANCH_NOEXIST)
            return

        if not c.user_is_loggedin or not sr.can_submit(c.user, self.promotion):
            self.set_error(errors.BRANCH_NOTALLOWED)
            return

        if not sr.allow_ads and self.promotion:
            self.set_error(errors.BRANCH_DISABLED_ADS)
            return
...
class VSubscribeSR(VByName):
    ...
    def run(self, sr_id, sr_name):
        ...
        try:
            sr = Branch._by_name(str(sr_name).strip())
        except (NotFound, AttributeError, UnicodeEncodeError):
            self.set_error(errors.BRANCH_NOEXIST)
            return
    ...
    def param_docs(self):
        return {
            self.param[0]: "the name of a branch",
        }
...
class VMessageRecipient(VExistingUname):
    def run(self, name):
        ...
        if is_branch:
            try:
                s = Branch._by_name(name)
                if isinstance(s, FakeBranch):
                    raise NotFound, "fake branch"
                if s._spam:
                    raise NotFound, "banned branch"
...
class VCssName(Validator):
    ...
    def param_docs(self):
        return {
            self.param: "a valid branch image name",
        }
...
class VDestination(Validator):
    ...
    def run(self, dest):
        ...
        if ld.startswith(('/', 'http://', 'https://')):
            ...
            if u.is_artemis_url(c.site) and u.is_web_safe_url():
                return dest
...
class VFlairTemplateByID(VRequired):
    ...
    def run(self, flair_template_id):
        try:
            return FlairTemplateByBranchIndex.get_template(
    ...
    class ArrayOf(object):
        ...
        def run(self, data):
            if not isinstance(data, list):
                raise ArtemisError('JSON_INVALID', code=400)
        ...
        def run(self, data, ignore_missing=False):
            ...
            for key, validator in self.spec.iteritems():
                try:
                    ...
                    if ignore_missing:
                        continue
                    raise ArtemisError('JSON_MISSING_KEY', code=400,
                                      msg_params={'key': key})
            return validated_data
...
class VMultiPath(Validator):
    """Validates a multiartemis path. Returns a path info dictionary.
    """
    ...
    def param_docs(self):
        return {
            self.param: "multiartemis url path",
        }


class VMultiByPath(Validator):
    """Validates a multiartemis path.  Returns a LabeledMulti.
    """
...
class VBranchList(Validator):
    ...
    def run(self, branches):
        ...
        # extract branch name if path provided
        branches = [sr_path_rx.sub('\g<name>', sr.strip())
                      for sr in branches.lower().strip().splitlines() if sr]
        ...
        unique_srs = set(branches)

        if branches:
            valid_srs = set(Branch._by_name(branches).keys())
            if unique_srs - valid_srs:
                return self.set_error(errors.BRANCH_NOEXIST, code=400)

        if len(unique_srs) > self.limit:
            return self.set_error(
                errors.TOO_MANY_BRANCHS, {'max': self.limit}, code=400)

        # return list of branch names as entered
        return branches

    def param_docs(self):
        return {
            self.param: 'a list of branch names, line break delimited',
        }
