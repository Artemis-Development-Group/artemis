...
from r2.models import Branch, NotFound
...
PREFS_VALIDATORS = dict(
    ...
    pref_media=VOneOf('media', ('on', 'off', 'branch')),
    # pref_media_preview=VOneOf('media_preview', ('on', 'off', 'branch')),
    ...
    pref_cartemis_autorenew=VBoolean("cartemis_autorenew"),
...
def set_prefs(user, prefs):
    for k, v in prefs.iteritems():
        if k == 'pref_beta' and v and not getattr(user, 'pref_beta', False):
            ...
            # to the beta branch.
            try:
                sr = Branch._by_name(g.beta_sr)
                if not sr.is_subscriber(user):
                    sr.add_subscriber(user)
            except NotFound:
                g.log.warning("Could not find beta branch '%s'. It may "
...
def filter_prefs(prefs, user):
    ...
    if (feature.is_enabled('stylesheets_everywhere', user=user) and
        ...
        override_sr = Branch._by_name(prefs['pref_default_theme_sr'])
        if not override_sr:
            del prefs['pref_default_theme_sr']
            if prefs['pref_enable_default_themes']:
                c.errors.add(c.errors.add(errors.BRANCH_REQUIRED, field="stylesheet_override"))
        else:
            if override_sr.can_view(user):
                prefs['pref_default_theme_sr'] = override_sr.name
            else:
                # don't update if they can't view the chosen branch
                c.errors.add(errors.BRANCH_NO_ACCESS, field='stylesheet_override')
                del prefs['pref_default_theme_sr']
