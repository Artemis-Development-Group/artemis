...
Currently only supports branch links but will soon support comment links.
"""
...
def _read_branch_etl_from_s3(s3path):
    s3conn = S3Connection()
...
def find_all_branches(s3path):
    for line in _read_branch_etl_from_s3(s3path):
        _, branch, __ = line.split('\x01')
        yield branch
