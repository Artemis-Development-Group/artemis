...
an external link or a link from a artemis, in general most artemis
links do not get indexed by search engines. This means that the vast
majority of Artemis content does not get indexed by Google, Bing, or Yahoo.
Sitemaps solve the problem by showing search engines links that they
likely don't have access to any other way.

Artemis contains tons and tons of links. Generating them on the fly is simply
impractical. The solution this module implements is a very slow batch that
goes through every branch and every link and creates crawlable permalinks
...
There are only two types of links we currently support. Branch links
in the form of
...
This module is split into 3 parts.
  ...
  r2.lib.sitemaps.data - Loads up the raw Branch and Link Things.
