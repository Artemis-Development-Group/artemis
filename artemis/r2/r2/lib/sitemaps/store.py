...
This module is uploads all branch sitemaps as well as the sitemap index
...
store_sitemaps_in_s3(branches)

Even though the branches are expected to be generated and passed into this
...
If we simply queried the branch iterator for it's length then we would
have to load all of the branches into memory, which would be ... bad.
"""
...
from r2.lib.sitemaps.generate import branch_sitemaps, sitemap_index
...
def store_branch_sitemap(bucket, index, sitemap):
    key = Key(bucket)
    key.key = 'branch_sitemap/{0}.xml'.format(index)
...
def store_sitemap_index(bucket, count):
    key = Key(bucket)
    key.key = g.sitemap_branch_keyname
    g.log.debug("Uploading %r", key)

    upload_sitemap(key, sitemap_index(count))


def store_sitemaps_in_s3(branches):
    s3conn = S3Connection()
    bucket = s3conn.get_bucket(g.sitemap_upload_s3_bucket, validate=False)

    sitemap_count = 0
    for i, sitemap in enumerate(branch_sitemaps(branches)):
        store_branch_sitemap(bucket, i, sitemap)
        sitemap_count += 1
