...
"""Create exhaustive sitemaps for Artemis.
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <sitemap>
    <loc>http://reddit.com/r/branch_sitemap?index=0</loc>
  </sitemap>
  <sitemap>
    <loc>http://reddit.com/r/branch_sitemap?index=1</loc>
  </sitemap>
  <sitemap>
...
Next are branch sitemaps which take the form of:
...
def _branch_links(branches):
    for branch in branches:
        path = '/r/{0}/'.format(branch)
        yield _absolute_url(path)


def _branch_sitemap(branches):
    urlset = etree.Element('urlset', xmlns=SITEMAP_NAMESPACE)
    for link in _branch_links(branches):
...
def branch_sitemaps(branches):
    ...
    """
    for branch_chunks in in_chunks(branches, LINKS_PER_SITEMAP):
        yield _branch_sitemap(branch_chunks)
...
def sitemap_index(count):
    ...
    for i in xrange(count):
        ...
        url = '{0}/branch_sitemap/{1}.xml'.format(
