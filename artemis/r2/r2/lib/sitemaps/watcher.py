...
from r2.lib.sitemaps.data import find_all_branches
...
def _branch_sitemap_key():
    conn = S3Connection()
    bucket = conn.get_bucket(g.sitemap_upload_s3_bucket, validate=False)
    return bucket.get_key(g.sitemap_branch_keyname)
...
def _before_last_sitemap(timestamp):
    sitemap_key = _branch_sitemap_key()
...
def _process_message():
    ...
    branches = find_all_branches(s3path)
...
def _create_test_message():
    ...
    message = sqs_q.new_message(body=json.dumps({
        ...
        'location': ('s3://artemis-data-analysis/big-data/r2/prod/' +
