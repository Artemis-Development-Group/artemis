...
error_list = dict((
        ...
        ('BRANCH_EXISTS', _('that branch already exists')),
        ('BRANCH_NOEXIST', _('that branch doesn\'t exist')),
        ('BRANCH_NOTALLOWED', _("you aren't allowed to post there.")),
        ('BRANCH_NO_ACCESS', _("you aren't allowed access to this branch")),
        ('BRANCH_REQUIRED', _('you must specify a branch')),
        ('BRANCH_DISABLED_ADS', _('this branch has chosen to disable their ads at this time')),
        ...
        ('BRANCH_RATELIMIT', _("you are doing that too much. try again later.")),
        ...
        ('OVERSOLD', _('that branch has already been oversold on %(start)s to %(end)s. Please pick another branch or date.')),
        ...
        ('NO_SELFS', _("that branch doesn't allow text posts")),
        ('NO_LINKS', _("that branch only allows text posts")),
        ...
        ('DOMAIN_BANNED', _('%(domain)s is not allowed on artemis: %(reason)s')),
        ...
        ('NO_INVITE_FOUND', _('there is no pending invite for that branch')),
        ...
        ('MULTI_NOT_FOUND', _('that multiartemis doesn\'t exist')),
        ('MULTI_EXISTS', _('that multiartemis already exists')),
        ('MULTI_CANNOT_EDIT', _('you can\'t change that multiartemis')),
        ('MULTI_TOO_MANY_BRANCHS', _('no more space for branches in that multiartemis')),
        ('MULTI_SPECIAL_BRANCH', _("can't add special branch %(path)s")),
        ('TOO_MANY_BRANCHS', _('maximum %(max)s branches')),
        ...
        ('BANNED_FROM_BRANCH', _('that user is banned from the branch')),
        ('IN_TIMEOUT', _("You can't do that while suspended.")),
        ('GOLD_REQUIRED', _('you must have an active artemis gold subscription to do that')),
        ('INSUFFICIENT_CARTEMISS', _("insufficient cbranches")),
        ('GILDING_NOT_ALLOWED', _("gilding is not allowed in this branch")),
        ('SCRAPER_ERROR', _("unable to scrape provided url")),
        ('NO_SR_TO_SR_MESSAGE', _("can't send a message from a branch to another branch")),
        ('USER_BLOCKED_MESSAGE', _("can't send message to that user")),
        ('ADMIN_REQUIRED', _("you must be in admin mode for this")),
        ('CANT_CONVERT_TO_GOLD_ONLY', _("to convert an existing branch to gold only, send a message to %(admin_modmail)s") 
            % dict(admin_modmail=g.admin_message_acct)),
        ('GOLD_ONLY_SR_REQUIRED', _("this branch must be 'gold only' to select this")),
        ('CANT_CREATE_SR', _("your account is too new or you do not have enough karma to create a branch. please contact the admins to request an exemption.")),
        ...
        ('USER_MUTED', _("You have been muted from this branch.")),
        ('MUTED_FROM_BRANCH', _("This user has been muted from the branch.")),
        ('COMMENT_NOT_STICKYABLE', _("This comment is not stickyable. Ensure that it is a top level comment.")),
        ('SR_RULE_EXISTS', _("A branch rule by that name already exists.")),
        ('SR_RULE_DOESNT_EXIST', _("No branch rule by that name exists.")),
        ('SR_RULE_TOO_MANY', _("This branch already has the maximum number of rules.")),
...
class ArtemisError(Exception):
    ...
    def __repr__(self):
        return '<ArtemisError: %s>' % self.name
...
class ErrorSet(object):
    ...
    def add(self, error_name, msg_params=None, field=None, code=None):
        for field_name in tup(field):
            e = ArtemisError(error_name, msg_params, fields=field_name,
                            code=code)
            self.add_error(e)
...
def artemis_http_error(code=400, error_name='UNKNOWN_ERROR', **data):
    exc = status_map[code]()
...
class UserRequiredException(ArtemisError):
    name = errors.USER_REQUIRED
    code = 403


class VerifiedUserRequiredException(ArtemisError):
