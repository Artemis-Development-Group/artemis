...
def js_config(extra_config=None):
    ...
    elif pref_media_preview == "branch":
        expando_preference = "branch_default"
    ...
    if isinstance(c.site, Branch) and not c.default_sr:
        cur_branch = c.site.name
        cur_sr_fullname = c.site._fullname
        cur_listing = cur_branch
    ...
    config = {
        ...
        # the branch's name (for posts)
        "post_site": cur_branch,
        ...
        # current domain
        "cur_domain": get_domain(branch=False, no_www=True),
        # where do ajax requests go?
        "ajax_domain": get_domain(branch=False),
        ...
        "is_fake": isinstance(c.site, FakeBranch),
...
def class_dict():
    t_cls = [Link, Comment, Message, Branch]
...
def get_domain(cname=False, branch=True, no_www=False):
    """
    returns the domain on the current branch, possibly including
    the branch part of the path, suitable for insertion after an
     ...
     * branch: flags whether or not to append to the domain the
       branch path (without the trailing path).
...
def add_sr(
    ...
    parses the path and updates it to include the branch path
    ...
    u = UrlParser(path)
    if sr_path:
        u.path_add_branch(c.site)

    if not u.hostname or force_hostname:
        u.hostname = get_domain(branch=False)

    if (c.secure and u.is_artemis_url()) or force_https:
        u.scheme = "https"
...
def add_attr(attrs, kind, label=None, link=None, cssclass=None, symbol=None):
    ...
    if kind == 'F':
        ...
        cssclass = 'admin'
        if not label:
            label = _('artemis admin, speaking officially')
    elif kind in ('X', '@'):
        priority = 5
        cssclass = 'gray'
        if not label:
...
def add_moderator_distinguish(distinguish_attribs_list, branch):
    link = '/r/%s/about/moderators' % branch.name
    label = _('moderator of /r/%(artemis)s, speaking officially')
    label %= {'artemis': branch.name}
...
def add_cakeday_distinguish(distinguish_attribs_list, user):
    label = _("%(user)s just celebrated a artemis birthday!")
...
def add_submitter_distinguish(distinguish_attribs_list, link, branch):
    permalink = link.make_permalink(branch)
    add_attr(distinguish_attribs_list, 'S', link=permalink)


def search_url(query, branch, restrict_sr="off", sort=None, recent=None, ref=None):
    import urllib
    ...
    if recent:
        url_query["t"] = recent
    path = "/r/%s/search?" % branch if branch else "/search?"
