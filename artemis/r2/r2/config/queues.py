...
def declare_queues(g):
    queues = Queues({
        ...
        "branch_query_q": MessageQueue(bind_to_self=True),
    ...
    if g.shard_branch_query_queues:
        sharded_branch_query_queues = {
            "branch_query_%d_q" % i: MessageQueue(bind_to_self=True)
            for i in xrange(10)
        }
        queues.declare(sharded_branch_query_queues)
