...
def error_mapper(code, message, environ, global_conf=None, **kw):
    ...
    if code in codes:
        ...
        if environ.get('ARTEMIS_NAME'):
            d['srname'] = environ.get('ARTEMIS_NAME')
        if environ.get('ARTEMIS_TAKEDOWN'):
            d['takedown'] = environ.get('ARTEMIS_TAKEDOWN')
        if environ.get('ARTEMIS_ERROR_NAME'):
            d['error_name'] = environ.get('ARTEMIS_ERROR_NAME')
...
class DomainMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        # tell artemis_base to redirect to the appropriate branch for
        # a legacy CNAME
        ...
        for subdomain in subdomains[:]:
            ...
            if subdomain in g.reserved_subdomains:
                ...
                environ['artemis-domain-extension'] = extension
            elif is_language_subdomain(subdomain):
                environ['artemis-prefer-lang'] = subdomain
            else:
                sr_redirect = subdomain
                subdomains.remove(subdomain)

        if 'artemis-prefer-lang' in environ:
            prefix_parts.insert(0, environ['artemis-prefer-lang'])
        if prefix_parts:
            environ['artemis-domain-prefix'] = '.'.join(prefix_parts)

        # if there was a branch subdomain, redirect
...
class ExtensionMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        domain_ext = environ.get('artemis-domain-extension')
...
class ArtemisApp(PylonsApp):

    test_mode = False

    def __init__(self, *args, **kwargs):
        super(ArtemisApp, self).__init__(*args, **kwargs)
    ...
    def _check_csrf_prevention(self):
        ...
        if not g.running_as_script:
            controllers_iter = itertools.chain(
                controllers._artemis_controllers.itervalues(),
