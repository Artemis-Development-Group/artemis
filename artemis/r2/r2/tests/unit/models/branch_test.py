...
from r2.models.branch import SRMember, Branch
...
class IsValidNameTest(unittest.TestCase):
    def test_empty(self):
        self.assertFalse(Branch.is_valid_name(None))

    def test_short(self):
        self.assertTrue(Branch.is_valid_name('aaa'))

    def test_too_short(self):
        self.assertFalse(Branch.is_valid_name('aa'))

    def test_long(self):
        self.assertTrue(Branch.is_valid_name('aaaaaaaaaaaaaaaaaaaaa'))

    def test_too_long(self):
        self.assertFalse(Branch.is_valid_name('aaaaaaaaaaaaaaaaaaaaaa'))

    def test_underscore(self):
        self.assertTrue(Branch.is_valid_name('a_a'))

    def test_leading_underscore(self):
        self.assertFalse(Branch.is_valid_name('_aa'))

    def test_capitals(self):
        self.assertTrue(Branch.is_valid_name('AZA'))

    def test_numerics(self):
        self.assertTrue(Branch.is_valid_name('090'))
...
class ByNameTest(unittest.TestCase):
    def setUp(self):
        ...
        self.branch_byID = MagicMock()
        Branch._byID = self.branch_byID

        self.branch_query = MagicMock()
        Branch._query = self.branch_query

    def testSingleCached(self):
        branch = Branch(id=1, name="exists")
        self.cache.get_multi.return_value = {"exists": branch._id}
        self.branch_byID.return_value = [branch]

        ret = Branch._by_name("exists")

        self.assertEqual(ret, branch)
        self.assertEqual(self.branch_query.call_count, 0)

    def testSingleFromDB(self):
        branch = Branch(id=1, name="exists")
        self.cache.get_multi.return_value = {}
        self.branch_query.return_value = [branch]
        self.branch_byID.return_value = [branch]
    ...
    def testSingleNotFound(self):
        self.cache.get_multi.return_value = {}
        self.branch_query.return_value = []

        with self.assertRaises(NotFound):
            Branch._by_name("doesnotexist")

    def testSingleInvalid(self):
        with self.assertRaises(NotFound):
            Branch._by_name("_illegalunderscore")
    ...
    def testMultiCached(self):
        srs = [
            Branch(id=1, name="exists"),
            Branch(id=2, name="also"),
        ]
        self.cache.get_multi.return_value = {sr.name: sr._id for sr in srs}
        self.branch_byID.return_value = srs

        ret = Branch._by_name(["exists", "also"])
    ...
    def testMultiCacheMissesAllExist(self):
        ...
        self.cache.get_multi.return_value = {}
        self.branch_query.return_value = srs
        self.branch_byID.return_value = srs
        ...
        self.assertEqual(self.branch_query.call_count, 1)

    def testMultiSomeDontExist(self):
        sr = Branch(id=1, name="exists")
        self.cache.get_multi.return_value = {sr.name: sr._id}
        self.branch_query.return_value = []
        self.branch_byID.return_value = [sr]

        ret = Branch._by_name(["exists", "doesnt"])
    ...
    def testMultiSomeInvalid(self):
        ...
        ret = Branch._by_name(["exists", "_illegalunderscore"])
    ...
    def testForceUpdate(self):
        sr = Branch(id=1, name="exists")
        self.cache.get_multi.return_value = {sr.name: sr._id}
        self.branch_query.return_value = [sr]
        self.branch_byID.return_value = [sr]

        ret = Branch._by_name("exists", _update=True)
    ...
    def testCacheNegativeResults(self):
        self.cache.get_multi.return_value = {}
        self.branch_query.return_value = []
        self.branch_byID.return_value = []

        with self.assertRaises(NotFound):
            Branch._by_name("doesnotexist")

        self.cache.set_multi.assert_called_once_with(
            keys={"doesnotexist": Branch.SRNAME_NOTFOUND},
            prefix="srid:",
            time=43200,
        )

    def testExcludeNegativeLookups(self):
        self.cache.get_multi.return_value = {"doesnotexist": Branch.SRNAME_NOTFOUND}

        with self.assertRaises(NotFound):
            Branch._by_name("doesnotexist")
        self.assertEqual(self.branch_query.call_count, 0)
        self.assertEqual(self.branch_byID.call_count, 0)
