...
from r2.models import Account, Comment, Link, Message, Branch


class ValidatorTests(ArtemisTestCase):
...
class TestVSubmitParent(ValidatorTests):
    def setUp(self):
        ...
        self.autopatch(Branch, "_byID", return_value=None)
    ...
    def _mock_link(self, id=1, author_id=1, sr_id=1, can_comment=True,
        ...
        sr = Branch(id=sr_id)
        self.autopatch(Branch, "_byID", return_value=sr)
        self.autopatch(Branch, "can_comment", return_value=can_comment)
    ...
    def _mock_comment(self,
        ...
        self.autopatch(Branch, "can_comment", return_value=can_comment)
        self.autopatch(Link, "can_view_promo", return_value=can_view_promo)
        self.autopatch(Branch, "is_moderator", return_value=is_moderator)
    ...
    def test_locked_link(self):
        ...
        with patch.object(Branch, "can_distinguish", return_value=False):
            result = self.validator.run('fullname', None)
    ...
    def test_locked_link_mod_reply(self):
        link = self._mock_link(locked=True)
        with patch.object(Branch, "can_distinguish", return_value=True):
            result = self.validator.run('fullname', None)
...
class TestVBranchName(ValidatorTests):
    def setUp(self):
        # Reset the validator state and errors before every test.
        self.validator = VBranchName(None)
        c.errors = ErrorSet()

    def _test_failure(self, input, error=errors.BAD_SR_NAME):
        super(TestVBranchName, self)._test_failure(input, error)
