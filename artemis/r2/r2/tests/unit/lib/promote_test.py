...
from r2.tests import ArtemisTestCase, NonCache


subscriptions_srnames = ["foo", "bar"]
subscriptions = map(lambda srname: Branch(name=srname), subscriptions_srnames)
multi_srnames = ["bing", "bat"]
multi_branches = map(lambda srname: Branch(name=srname), multi_srnames)
nice_srname = "mylittlepony"
nsfw_srname = "pr0n"
questionably_nsfw = "sexstories"
quarantined_srname = "croontown"
naughty_subscriptions = [
    Branch(name=nice_srname),
    Branch(name=nsfw_srname, over_18=True),
    Branch(name=quarantined_srname, quarantine=True),
]
...
class TestSRNamesFromSite(ArtemisTestCase):
    def setUp(self):
    ...
    @patch("r2.models.Branch.user_branches")
    def test_frontpage_logged_in(self, user_branches):
        user_branches.return_value = subscriptions
    ...
    def test_multi_logged_out(self):
        multi = MultiArtemis(path="/user/test/m/multi_test", srs=multi_branches)
        srnames = srnames_from_site(self.logged_out, multi)

        self.assertEqual(srnames, set(multi_srnames))

    @patch("r2.models.Branch.user_branches")
    def test_multi_logged_in(self, user_branches):
        user_branches.return_value = subscriptions
        multi = MultiArtemis(path="/user/test/m/multi_test", srs=multi_branches)
    ...
    def test_branch_logged_out(self):
        srname = "test1"
        branch = Branch(name=srname)
        srnames = srnames_from_site(self.logged_out, branch)

        self.assertEqual(srnames, {srname})

    @patch("r2.models.Branch.user_branches")
    def test_branch_logged_in(self, user_branches):
        user_branches.return_value = subscriptions
        srname = "test1"
        branch = Branch(name=srname)
        srnames = srnames_from_site(self.logged_in, branch)

        self.assertEqual(srnames, {srname})

    @patch("r2.models.Branch.user_branches")
    def test_quarantined_subscriptions_are_never_included(self, user_branches):
        user_branches.return_value = naughty_subscriptions
        branch = Frontpage
        srnames = srnames_from_site(self.logged_in, branch)

        self.assertEqual(srnames, {branch.name} | {nice_srname})
        self.assertTrue(len(srnames & {quarantined_srname}) == 0)

    @patch("r2.models.Branch.user_branches")
    def test_nsfw_subscriptions_arent_included_when_viewing_frontpage(self, user_branches):
        user_branches.return_value = naughty_subscriptions
        srnames = srnames_from_site(self.logged_in, Frontpage)
    ...
    def test_remove_nsfw_collection_srnames_on_frontpage(self, get_nsfw_collections_srnames):
        ...
        srname = "test1"
        branch = Branch(name=srname)
        Branch.user_branches = MagicMock(return_value=[
            Branch(name=nice_srname),
            Branch(name=questionably_nsfw),
        ])

        frontpage_srnames = srnames_from_site(self.logged_in, Frontpage)
        swf_srnames = srnames_from_site(self.logged_in, branch)
