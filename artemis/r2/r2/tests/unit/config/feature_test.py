...
class TestFeatureBase(ArtemisTestCase):
    _world = None
...
class TestFeatureBase(ArtemisTestCase):
    ...
    def setUp(self):
        ...
        self.world.current_branch = mock.Mock(return_value='')
...
class TestFeature(TestFeatureBase):
    ...
    def test_branch_in(self):
        cfg = {'branches': ['WTF']}
        feature_state = self.world._make_state(cfg)
        self.assertTrue(feature_state.is_enabled(branch='wtf'))

        cfg = {'branches': ['wtf']}
        feature_state = self.world._make_state(cfg)
        self.assertTrue(feature_state.is_enabled(branch='WTF'))

        cfg = {'branches': ['aww', 'wtf']}
        feature_state = self.world._make_state(cfg)
        self.assertTrue(feature_state.is_enabled(branch='wtf'))

    def test_branch_not_in(self):
        cfg = {'branches': []}
        feature_state = self.world._make_state(cfg)
        self.assertFalse(feature_state.is_enabled(branch='wtf'))

        cfg = {'branches': ['aww', 'wtfoobar']}
        feature_state = self.world._make_state(cfg)
        self.assertFalse(feature_state.is_enabled(branch='wtf'))
