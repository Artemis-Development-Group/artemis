...
__all__ = ['ArtemisTestCase', 'ArtemisControllerTestCase']
...
from r2.config.middleware import ArtemisApp
...
class ArtemisTestCase(TestCase):
    ...
    def setUp(self):
        ...
        def reset_test_mode(orig_value=ArtemisApp.test_mode):
            ArtemisApp.test_mode = orig_value
        self.addCleanup(reset_test_mode)
        ArtemisApp.test_mode = True
...
class ArtemisControllerTestCase(ArtemisTestCase):
    CONTROLLER = None
    ACTIONS = {}

    def setUp(self):
        super(ArtemisControllerTestCase, self).setUp()
        ...
        # to work properly
        for i, _cls in enumerate((Link, Branch, Account)):
            ...
            if not hasattr(_cls, "_type_name"):
                self.autopatch(
                    ...
                    create=True)
        # The same is true for _by_name on Branch and Account
        self.branch_by_name = self.autopatch(Branch, "_by_name")
