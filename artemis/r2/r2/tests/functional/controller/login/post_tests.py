...
###############################################################################
from r2.tests import ArtemisControllerTestCase
...
class PostLoginRegTests(LoginRegBase, ArtemisControllerTestCase):
    CONTROLLER = "post"
    ACTIONS = {
        "register": "reg",
    }
    ...
    def setUp(self):
        ...
        self.autopatch(Branch, "_byID", return_value=[])
        self.dest = "/foo"
    ...
    def assert_success(self, res):
        ...
        self.assert_headers(
            res,
            "Set-Cookie",
            lambda value: value.startswith("artemis_session=")
        )
