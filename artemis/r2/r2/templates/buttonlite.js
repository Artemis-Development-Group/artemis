...
##
## All portions of the code written by artemis are Copyright (c) 2006-2015
## artemis Inc. All Rights Reserved.
###############################################################################
...
<%def name="submiturl(url, title='')">${("%s://%s/submit" % (g.default_scheme, get_domain(branch=True))) + query_string(dict(url=url, title=title))}</%def>
...
(function() {
    ...
    var write_string='<span class="artemis_button" style="';
%if thing.styled:    
    write_string += 'color: grey;';
%endif
    write_string += '">';
%if thing.image > 0:
    write_string += unstyled_submit + '<img style="height: 2.3ex; vertical-align:top; margin-right: 1ex" src="${static('spartemis' + str(thing.image) + '.gif')}">' + "</a>";
%endif
%if thing._fullname:
    write_string += '${jssafe(websafe(Score.safepoints(thing.score)))}';
    %if thing.styled:  
        write_string += ' on ' + styled_submit + 'artemis</a>';
    %else:
        write_string += ' on ' + unstyled_submit + 'artemis</a>';
    %endif
%else:
    %if thing.styled:
