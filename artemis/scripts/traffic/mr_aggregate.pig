...
-- branch --
branch = LOAD '$INPUT/branch' AS (branch, unique_id, count:long);

branch_grouped = GROUP branch BY (branch, unique_id);
branch_combined = FOREACH branch_grouped
                        GENERATE group.branch AS branch,
                                 group.unique_id AS unique_id,
                                 SUM(branch.count) AS count;

branch_grouped2 = GROUP branch_combined BY branch;
branch_totals = FOREACH branch_grouped2
                      GENERATE group,
                      COUNT(branch_combined),
                      SUM(branch_combined.count);

STORE branch_totals INTO '$OUTPUT/branch';
