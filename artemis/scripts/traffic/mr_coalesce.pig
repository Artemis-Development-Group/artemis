...
-- branch
branch_counters = LOAD '$INPUT/branch' AS (branch, unique_id, count:long);

branches_grouped = GROUP branch_counters BY (branch, unique_id);

branches_coalesced = FOREACH branches_grouped
                       GENERATE group.branch, group.unique_id,
                                SUM(branch_counters.count) AS count;

STORE branches_coalesced INTO '$OUTPUT/branch';
