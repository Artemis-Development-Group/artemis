 ...
 *  for each category sitewide/branch/srpath/lang/clicks/clicks_targeted/thing/thingtarget
 *  generate output with entries like:
 *  category, unique_id, count (e.g. branch: pics, 123456, 9)
 *
...
pageviews = STREAM pageviews_encrypted THROUGH DECRYPT_USERINFO AS (unique_id, srpath, branch, lang, cname);
...
-- branch
branch_pageviews_filtered = FILTER pageviews 
                               BY branch IS NOT NULL;  -- exclude entries without branch
branch_pageviews_raw = FOREACH branch_pageviews_filtered 
                          GENERATE branch, unique_id;  -- limit to (branch, unique_id)
branch_hourly_uniques_grouped = GROUP branch_pageviews_raw
                                   BY (branch, unique_id); -- (branch, unique_id, {(branch, unique_id), ...})
branch_hourly_uniques = FOREACH branch_hourly_uniques_grouped
                           GENERATE group.branch, group.unique_id,
                                    COUNT(branch_pageviews_raw) AS count; -- (branch, unique_id, count)

STORE branch_hourly_uniques INTO '$OUTPUT/branch';
