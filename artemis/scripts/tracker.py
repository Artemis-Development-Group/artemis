...
artemis_domain = config.get('DEFAULT', 'domain')
artemis_domain_prefix = config.get('DEFAULT', 'domain_prefix')
...
def event_redirect():
    ...
    netloc = "%s.%s" % (artemis_domain_prefix, artemis_domain)
...
def event_click():
    ...
    that does not have session information - i.e. served from artemismedia.com.
    ...
    """
    try:
        session_str = urllib.unquote(request.cookies.get('artemis_session', ''))
