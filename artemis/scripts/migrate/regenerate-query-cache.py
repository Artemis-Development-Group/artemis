...
relations = {"savehide": ("UserQueryCache", "link"),
             ...
             "moderatorinbox": ("BranchQueryCache", "message"),
...
FOREACH grouped_with_data
    GENERATE FLATTEN(items),
             com.artemis.pig.MAKE_MAP(data.(key, value)) AS data;
"""
...
potential_columns =
FOREACH only_valid
    GENERATE com.artemis.pig.MAKE_ROWKEY(relation, name, thing1_id) AS rowkey,
             com.artemis.pig.MAKE_THING2_FULLNAME(relation, thing2_id) AS colkey,
             timestamp AS value;
"""
...
FOREACH limited GENERATE rowkey,
                         colkey,
                         com.artemis.pig.TO_JSON(value);

STORE jsonified INTO '$OUTPUT' USING PigStorage();
"""

###### run the jobs
# register the artemis udfs
Pig.registerJar(SCRIPT_ROOT + "artemis-pig-udfs.jar")
...
if False:
    ...
    FOREACH non_null
    GENERATE
        CONCAT('sent.', com.artemis.pig.TO_36(data#'author_id')) AS rowkey,
        com.artemis.pig.MAKE_FULLNAME('message', id) AS colkey,
        timestamp AS value;
    """
    script += store_top_1000_per_rowkey
    compiled = Pig.compile(script)
...
if True:
    ...
    FOREACH comments_with_name GENERATE
        CONCAT(name, CONCAT('.', com.artemis.pig.TO_36(data#'sr_id'))) AS rowkey,
        com.artemis.pig.MAKE_FULLNAME('comment', id) AS colkey,
        timestamp AS value;
    """
    script += store_top_1000_per_rowkey
    compiled = Pig.compile(script)
    bound = compiled.bind({
        ...
        "OUTPUT": "/".join((OUTPUT_ROOT, "BranchQueryCache", "comment")),
    })
    result = bound.runSingle()
